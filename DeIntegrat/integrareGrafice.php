<?php $data = require_once('./_Bar/getData.php'); ?>

<!-- Resources -->
<script src="./libraries/core.js"></script>
<script src="./libraries/charts.js"></script>
<script src="./libraries/dataviz.js"></script>
<script src="./libraries/animated.js"></script>

<!-- Chart code -->
<script>
am4core.ready(function() {

// Themes begin
am4core.useTheme(am4themes_dataviz);
am4core.useTheme(am4themes_animated);
// Themes end

// Create chart instance
var chart = am4core.create("Bar", am4charts.XYChart);

// Add data
chart.data = <?=$data?>;
// Create axes

var categoryAxis = chart.xAxes.push(new am4charts.CategoryAxis());
categoryAxis.dataFields.category = "numeTara";
categoryAxis.renderer.grid.template.location = 0;
categoryAxis.renderer.minGridDistance = 30;

categoryAxis.renderer.labels.template.adapter.add("dy", function(dy, target) {
  if (target.dataItem && target.dataItem.index & 2 == 2) {
    return dy + 25;
  }
  return dy;
});

var valueAxis = chart.yAxes.push(new am4charts.ValueAxis());

// Create series
var series = chart.series.push(new am4charts.ColumnSeries());
series.dataFields.valueY = "numarAtacuri";
series.dataFields.categoryX = "numeTara";
series.name = "Attacks";
series.columns.template.tooltipText = "{categoryX}: [bold]{valueY}[/]";
series.columns.template.fillOpacity = .8;

var columnTemplate = series.columns.template;
columnTemplate.strokeWidth = 2;
columnTemplate.strokeOpacity = 1;

}); // end am4core.ready()
</script>

<?php $data = require_once('./_Line/getData.php'); ?>

<!-- Resources -->
<script src="https://cdn.amcharts.com/lib/4/core.js"></script>
<script src="https://cdn.amcharts.com/lib/4/charts.js"></script>
<script src="https://cdn.amcharts.com/lib/4/themes/dataviz.js"></script>
<script src="https://cdn.amcharts.com/lib/4/themes/animated.js"></script>

<!-- Chart code -->
<script>
    am4core.ready(function() {

        // Themes begin
        am4core.useTheme(am4themes_dataviz);
        am4core.useTheme(am4themes_animated);
        // Themes end

        var chart = am4core.create("Line", am4charts.XYChart);

        var data = [];
        var value = 50;
        chart.data = <?= $data ?>;

        // Create axes
        var dateAxis = chart.xAxes.push(new am4charts.DateAxis());
        dateAxis.renderer.minGridDistance = 60;

        var valueAxis = chart.yAxes.push(new am4charts.ValueAxis());

        // Create series
        var series = chart.series.push(new am4charts.LineSeries());
        series.dataFields.valueY = "numarAtacuri";
        series.dataFields.dateX = "an";
        series.tooltipText = "{numarAtacuri}"

        series.tooltip.pointerOrientation = "vertical";

        chart.cursor = new am4charts.XYCursor();
        chart.cursor.snapToSeries = series;
        chart.cursor.xAxis = dateAxis;

        //chart.scrollbarY = new am4core.Scrollbar();
        chart.scrollbarX = new am4core.Scrollbar();

    }); // end am4core.ready()
</script>

<?php $data = require_once('./_PieChart/getData.php'); ?>

<!-- Resources -->
<script src="libraries/core.js"></script>
<script src="libraries/charts.js"></script>
<script src="libraries/dataviz.js"></script>

<!-- Chart code -->
<script>
    am4core.ready(function() {

        // Themes begin
        am4core.useTheme(am4themes_dataviz);
        // Themes end

        var chart = am4core.create("Pie", am4charts.PieChart);
        chart.hiddenState.properties.opacity = 0; // this creates initial fade-in

        chart.legend = new am4charts.Legend();
        var legendContainer = am4core.create("PieLeg", am4core.Container);
        chart.legend.parent = legendContainer;

        legendContainer.width = am4core.percent(100);


        chart.data = <?= $data ?>;

        chart.innerRadius = 100;

        var series = chart.series.push(new am4charts.PieSeries());
        series.dataFields.value = "numarAtacuri";
        series.dataFields.category = "numeTara";

        series.labels.template.maxWidth = 130;
        series.labels.template.wrap = true;

        chart.exporting.menu = new am4core.ExportMenu();
        chart.exporting.menu.items[0].icon = "SaveIcon.png";
        chart.exporting.menu.align = "left";
        chart.exporting.menu.verticalAlign = "top";
        /*chart.exporting.formatOptions.getKey("jpg").disabled = true;
        chart.exporting.formatOptions.getKey("pdf").disabled = true;
        chart.exporting.formatOptions.getKey("json").disabled = true;
        chart.exporting.formatOptions.getKey("html").disabled = true;
        chart.exporting.formatOptions.getKey("xlsx").disabled = true;
        chart.exporting.formatOptions.getKey("print").disabled = true;*/
        //series.alignLabels = false;

    }); // end am4core.ready()
</script>

<!DOCTYPE html>
<html lang="en">

<head>
    <link rel="icon" href="../resurse/Poze/icon.jpg">
    <link href="integrareGrafice.css" rel="stylesheet">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta charset="utf-8">
    <title>Statistics</title>
</head>

<body style="background-color:#35e7ac;">

    <nav id="bara">
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div><a href="../public/map" class="Button">Map</a></div>
        <div><a href="../public/home" class="Button">Home</a></div>
        <div><a href="../public/statistics" class="Button">Statistics</a></div>
        <div><a href="../public/contact" class="Button">Contact</a></div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
    </nav>

    <main id="pagina">
        <form action="Test.php" method="GET">

            <div class="search">
                <input id="baralatime" type="text" name="searchsummary" placeholder="Search...">
                <input type="submit" class="butonSee" value="Search">
            </div>

            <div class="filters">

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="iyearin">An inceput:</label>
                    </div>
                    <div class="filteritem">
                        <input type="number" id="iyearin" name="iyearin">
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="iyearsf">An sfarsit:</label>
                    </div>
                    <div class="filteritem">
                        <input type="number" id="iyearsf" name="iyearsf">
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="country_txt">Tara:</label>
                    </div>
                    <div class="filteritem">
                        <select id="country_txt" name="country_txt">
                            <option></option>
                            <option value="Dominican Republic">Dominican Republic</option>
                            <option value="Mexico">Mexico</option>
                            <option value="Philippines">Philippines</option>
                            <option value="Greece">Greece</option>
                            <option value="Japan">Japan</option>
                            <option value="United States">United States</option>
                            <option value="Uruguay">Uruguay</option>
                            <option value="Italy">Italy</option>
                            <option value="East Germany (GDR)">East Germany (GDR)</option>
                            <option value="Ethiopia">Ethiopia</option>
                            <option value="Guatemala">Guatemala</option>
                            <option value="Venezuela">Venezuela</option>
                            <option value="West Germany (FRG)">West Germany (FRG)</option>
                            <option value="Switzerland">Switzerland</option>
                            <option value="Jordan">Jordan</option>
                            <option value="Spain">Spain</option>
                            <option value="Brazil">Brazil</option>
                            <option value="Egypt">Egypt</option>
                            <option value="Argentina">Argentina</option>
                            <option value="Lebanon">Lebanon</option>
                            <option value="Ireland">Ireland</option>
                            <option value="Turkey">Turkey</option>
                            <option value="Paraguay">Paraguay</option>
                            <option value="Iran">Iran</option>
                            <option value="United Kingdom">United Kingdom</option>
                            <option value="Colombia">Colombia</option>
                            <option value="Bolivia">Bolivia</option>
                            <option value="Nicaragua">Nicaragua</option>
                            <option value="Netherlands">Netherlands</option>
                            <option value="Belgium">Belgium</option>
                            <option value="Canada">Canada</option>
                            <option value="Australia">Australia</option>
                            <option value="Pakistan">Pakistan</option>
                            <option value="Zambia">Zambia</option>
                            <option value="Sweden">Sweden</option>
                            <option value="Costa Rica">Costa Rica</option>
                            <option value="South Yemen">South Yemen</option>
                            <option value="Cambodia">Cambodia</option>
                            <option value="Israel">Israel</option>
                            <option value="Poland">Poland</option>
                            <option value="Taiwan">Taiwan</option>
                            <option value="Panama">Panama</option>
                            <option value="Kuwait">Kuwait</option>
                            <option value="West Bank and Gaza Strip">West Bank and Gaza Strip</option>
                            <option value="Austria">Austria</option>
                            <option value="Czechoslovakia">Czechoslovakia</option>
                            <option value="India">India</option>
                            <option value="France">France</option>
                            <option value="South Vietnam">South Vietnam</option>
                            <option value="Brunei">Brunei</option>
                            <option value="Zaire">Zaire</option>
                            <option value="People\s Republic of the Congo">People\s Republic of the Congo</option>
                            <option value="Portugal">Portugal</option>
                            <option value="Algeria">Algeria</option>
                            <option value="El Salvador">El Salvador</option>
                            <option value="Thailand">Thailand</option>
                            <option value="Haiti">Haiti</option>
                            <option value="Sudan">Sudan</option>
                            <option value="Morocco">Morocco</option>
                            <option value="Cyprus">Cyprus</option>
                            <option value="Myanmar">Myanmar</option>
                            <option value="Afghanistan">Afghanistan</option>
                            <option value="Peru">Peru</option>
                            <option value="Chile">Chile</option>
                            <option value="Honduras">Honduras</option>
                            <option value="Yugoslavia">Yugoslavia</option>
                            <option value="Ecuador">Ecuador</option>
                            <option value="New Zealand">New Zealand</option>
                            <option value="Malaysia">Malaysia</option>
                            <option value="Singapore">Singapore</option>
                            <option value="Botswana">Botswana</option>
                            <option value="Jamaica">Jamaica</option>
                            <option value="Chad">Chad</option>
                            <option value="North Yemen">North Yemen</option>
                            <option value="Andorra">Andorra</option>
                            <option value="Syria">Syria</option>
                            <option value="South Korea">South Korea</option>
                            <option value="United Arab Emirates">United Arab Emirates</option>
                            <option value="South Africa">South Africa</option>
                            <option value="Kenya">Kenya</option>
                            <option value="Iraq">Iraq</option>
                            <option value="Somalia">Somalia</option>
                            <option value="Tanzania">Tanzania</option>
                            <option value="Sri Lanka">Sri Lanka</option>
                            <option value="Namibia">Namibia</option>
                            <option value="Bahamas">Bahamas</option>
                            <option value="Nigeria">Nigeria</option>
                            <option value="Barbados">Barbados</option>
                            <option value="Trinidad and Tobago">Trinidad and Tobago</option>
                            <option value="Bangladesh">Bangladesh</option>
                            <option value="Angola">Angola</option>
                            <option value="Mauritania">Mauritania</option>
                            <option value="Saudi Arabia">Saudi Arabia</option>
                            <option value="Djibouti">Djibouti</option>
                            <option value="Indonesia">Indonesia</option>
                            <option value="Malta">Malta</option>
                            <option value="Rhodesia">Rhodesia</option>
                            <option value="Soviet Union">Soviet Union</option>
                            <option value="Denmark">Denmark</option>
                            <option value="Western Sahara">Western Sahara</option>
                            <option value="Guyana">Guyana</option>
                            <option value="Mozambique">Mozambique</option>
                            <option value="Tunisia">Tunisia</option>
                            <option value="Uganda">Uganda</option>
                            <option value="Norway">Norway</option>
                            <option value="Lesotho">Lesotho</option>
                            <option value="Gabon">Gabon</option>
                            <option value="Libya">Libya</option>
                            <option value="Bahrain">Bahrain</option>
                            <option value="Hong Kong">Hong Kong</option>
                            <option value="Senegal">Senegal</option>
                            <option value="Zimbabwe">Zimbabwe</option>
                            <option value="Guinea">Guinea</option>
                            <option value="Grenada">Grenada</option>
                            <option value="New Hebrides">New Hebrides</option>
                            <option value="Belize">Belize</option>
                            <option value="Guadeloupe">Guadeloupe</option>
                            <option value="Martinique">Martinique</option>
                            <option value="Vatican City">Vatican City</option>
                            <option value="Albania">Albania</option>
                            <option value="Central African Republic">Central African Republic</option>
                            <option value="Seychelles">Seychelles</option>
                            <option value="Dominica">Dominica</option>
                            <option value="Qatar">Qatar</option>
                            <option value="Bulgaria">Bulgaria</option>
                            <option value="Suriname">Suriname</option>
                            <option value="Swaziland">Swaziland</option>
                            <option value="Luxembourg">Luxembourg</option>
                            <option value="Iceland">Iceland</option>
                            <option value="French Guiana">French Guiana</option>
                            <option value="Falkland Islands">Falkland Islands</option>
                            <option value="Burkina Faso">Burkina Faso</option>
                            <option value="New Caledonia">New Caledonia</option>
                            <option value="Romania">Romania</option>
                            <option value="Niger">Niger</option>
                            <option value="Nepal">Nepal</option>
                            <option value="Togo">Togo</option>
                            <option value="Finland">Finland</option>
                            <option value="Fiji">Fiji</option>
                            <option value="Ghana">Ghana</option>
                            <option value="Maldives">Maldives</option>
                            <option value="Mauritius">Mauritius</option>
                            <option value="Hungary">Hungary</option>
                            <option value="Laos">Laos</option>
                            <option value="Papua New Guinea">Papua New Guinea</option>
                            <option value="China">China</option>
                            <option value="Liberia">Liberia</option>
                            <option value="Republic of the Congo">Republic of the Congo</option>
                            <option value="Mali">Mali</option>
                            <option value="Germany">Germany</option>
                            <option value="Yemen">Yemen</option>
                            <option value="Rwanda">Rwanda</option>
                            <option value="Sierra Leone">Sierra Leone</option>
                            <option value="Cameroon">Cameroon</option>
                            <option value="Cuba">Cuba</option>
                            <option value="Croatia">Croatia</option>
                            <option value="Georgia">Georgia</option>
                            <option value="Azerbaijan">Azerbaijan</option>
                            <option value="Madagascar">Madagascar</option>
                            <option value="Lithuania">Lithuania</option>
                            <option value="Burundi">Burundi</option>
                            <option value="Ukraine">Ukraine</option>
                            <option value="Moldova">Moldova</option>
                            <option value="Armenia">Armenia</option>
                            <option value="Russia">Russia</option>
                            <option value="Ivory Coast">Ivory Coast</option>
                            <option value="Kazakhstan">Kazakhstan</option>
                            <option value="Antigua and Barbuda">Antigua and Barbuda</option>
                            <option value="Bosnia-Herzegovina">Bosnia-Herzegovina</option>
                            <option value="Equatorial Guinea">Equatorial Guinea</option>
                            <option value="Tajikistan">Tajikistan</option>
                            <option value="Malawi">Malawi</option>
                            <option value="Uzbekistan">Uzbekistan</option>
                            <option value="Latvia">Latvia</option>
                            <option value="Estonia">Estonia</option>
                            <option value="Vietnam">Vietnam</option>
                            <option value="Comoros">Comoros</option>
                            <option value="Benin">Benin</option>
                            <option value="Slovak Republic">Slovak Republic</option>
                            <option value="Macedonia">Macedonia</option>
                            <option value="Wallis and Futuna">Wallis and Futuna</option>
                            <option value="Belarus">Belarus</option>
                            <option value="Czech Republic">Czech Republic</option>
                            <option value="Slovenia">Slovenia</option>
                            <option value="Gambia">Gambia</option>
                            <option value="North Korea">North Korea</option>
                            <option value="Eritrea">Eritrea</option>
                            <option value="St. Kitts and Nevis">St. Kitts and Nevis</option>
                            <option value="French Polynesia">French Polynesia</option>
                            <option value="Macau">Macau</option>
                            <option value="Kyrgyzstan">Kyrgyzstan</option>
                            <option value="Vanuatu">Vanuatu</option>
                            <option value="Democratic Republic of the Congo">Democratic Republic of the Congo</option>
                            <option value="Kosovo">Kosovo</option>
                            <option value="Solomon Islands">Solomon Islands</option>
                            <option value="East Timor">East Timor</option>
                            <option value="St. Lucia">St. Lucia</option>
                            <option value="Guinea-Bissau">Guinea-Bissau</option>
                            <option value="Montenegro">Montenegro</option>
                            <option value="International">International</option>
                            <option value="Turkmenistan">Turkmenistan</option>
                            <option value="Serbia-Montenegro">Serbia-Montenegro</option>
                            <option value="Bhutan">Bhutan</option>
                            <option value="Serbia">Serbia</option>
                            <option value="South Sudan">South Sudan</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="region_txt">Regiune:</label>
                    </div>
                    <div class="filteritem">
                        <select id="region_txt" name="region_txt">
                            <option></option>
                            <option value="Central America & Caribbean">Central America & Caribbean</option>
                            <option value="North America">North America</option>
                            <option value="Southeast Asia">Southeast Asia</option>
                            <option value="Western Europe">Western Europe</option>
                            <option value="East Asia">East Asia</option>
                            <option value="South America">South America</option>
                            <option value="Eastern Europe">Eastern Europe</option>
                            <option value="Sub-Saharan Africa">Sub-Saharan Africa</option>
                            <option value="Middle East & North Africa">Middle East & North Africa</option>
                            <option value="Australasia & Oceania">Australasia & Oceania</option>
                            <option value="South Asia">South Asia</option>
                            <option value="Central Asia">Central Asia</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="provstate">Judet:</label>
                    </div>
                    <div class="filteritem">
                        <select id="provstate" name="provstate">
                            <option></option>
                            <option value="Federal">Federal</option>
                            <option value="Tarlac">Tarlac</option>
                            <option value="Attica">Attica</option>
                            <option value="Fukouka">Fukouka</option>
                            <option value="Illinois">Illinois</option>
                            <option value="Montevideo">Montevideo</option>
                            <option value="California">California</option>
                            <option value="Wisconsin">Wisconsin</option>
                            <option value="Colorado">Colorado</option>
                            <option value="Lazio">Lazio</option>
                            <option value="Michigan">Michigan</option>
                            <option value="Puerto Rico">Puerto Rico</option>
                            <option value="Berlin">Berlin</option>
                            <option value="Unknown">Unknown</option>
                            <option value="New York">New York</option>
                            <option value="Washington">Washington</option>
                            <option value="New Jersey">New Jersey</option>
                            <option value="Guatemala">Guatemala</option>
                            <option value="Metropolitan Manila">Metropolitan Manila</option>
                            <option value="Caracas">Caracas</option>
                            <option value="Nebraska">Nebraska</option>
                            <option value="Mississippi">Mississippi</option>
                            <option value="Ohio">Ohio</option>
                            <option value="Florida">Florida</option>
                            <option value="Oregon">Oregon</option>
                            <option value="Massachusetts">Massachusetts</option>
                            <option value="Bavaria">Bavaria</option>
                            <option value="Maryland">Maryland</option>
                            <option value="Louisiana">Louisiana</option>
                            <option value="Connecticut">Connecticut</option>
                            <option value="Hesse">Hesse</option>
                            <option value="Zurich">Zurich</option>
                            <option value="Arizona">Arizona</option>
                            <option value="Khalil">Khalil</option>
                            <option value="Metropolitian Manila">Metropolitian Manila</option>
                            <option value="Alabama">Alabama</option>
                            <option value="New Mexico">New Mexico</option>
                            <option value="Sao Paulo">Sao Paulo</option>
                            <option value="Alexandria">Alexandria</option>
                            <option value="Montana">Montana</option>
                            <option value="District of Columbia">District of Columbia</option>
                            <option value="Pampanga">Pampanga</option>
                            <option value="National">National</option>
                            <option value="Corrientes">Corrientes</option>
                            <option value="Buenos Aires">Buenos Aires</option>
                            <option value="Beirut">Beirut</option>
                            <option value="Georgia">Georgia</option>
                            <option value="Texas">Texas</option>
                            <option value="Dublin">Dublin</option>
                            <option value="Rio Grande do Sul">Rio Grande do Sul</option>
                            <option value="Kansas">Kansas</option>
                            <option value="Missouri">Missouri</option>
                            <option value="Amman">Amman</option>
                            <option value="Istanbul">Istanbul</option>
                            <option value="Pennsylvania">Pennsylvania</option>
                            <option value="Minnesota">Minnesota</option>
                            <option value="Iowa">Iowa</option>
                            <option value="Indiana">Indiana</option>
                            <option value="Asuncion">Asuncion</option>
                            <option value="North Carolina">North Carolina</option>
                            <option value="Oklahoma">Oklahoma</option>
                            <option value="Idaho">Idaho</option>
                            <option value="Geneva">Geneva</option>
                            <option value="Utah">Utah</option>
                            <option value="Capiz">Capiz</option>
                            <option value="Lara">Lara</option>
                            <option value="Tehran">Tehran</option>
                            <option value="Rio de Janeiro">Rio de Janeiro</option>
                            <option value="Northern Ireland">Northern Ireland</option>
                            <option value="Caldas">Caldas</option>
                            <option value="Managua">Managua</option>
                            <option value="Pernambuco">Pernambuco</option>
                            <option value="Navarre">Navarre</option>
                            <option value="North Holland">North Holland</option>
                            <option value="Brussels">Brussels</option>
                            <option value="Tucuman">Tucuman</option>
                            <option value="Santa Fe">Santa Fe</option>
                            <option value="Ankara">Ankara</option>
                            <option value="Tennessee">Tennessee</option>
                            <option value="Cordoba">Cordoba</option>
                            <option value="England">England</option>
                            <option value="Izmir">Izmir</option>
                            <option value="Quebec">Quebec</option>
                            <option value="West Virginia">West Virginia</option>
                            <option value="Baden-Wurttemberg">Baden-Wurttemberg</option>
                            <option value="Victoria">Victoria</option>
                            <option value="Sindh">Sindh</option>
                            <option value="Gipuzcoa">Gipuzcoa</option>
                            <option value="Burgos">Burgos</option>
                            <option value="Malatya">Malatya</option>
                            <option value="Amhara">Amhara</option>
                            <option value="Gilan">Gilan</option>
                            <option value="Vastra Gotaland">Vastra Gotaland</option>
                            <option value="San Jose">San Jose</option>
                            <option value="Hawaii">Hawaii</option>
                            <option value="South Holland">South Holland</option>
                            <option value="Santa Cruz de Tenerife">Santa Cruz de Tenerife</option>
                            <option value="Lombardy">Lombardy</option>
                            <option value="Esfahan">Esfahan</option>
                            <option value="Madrid">Madrid</option>
                            <option value="Barcelona">Barcelona</option>
                            <option value="Stockholm">Stockholm</option>
                            <option value="La Paz">La Paz</option>
                            <option value="Nevada">Nevada</option>
                            <option value="Ionian Islands">Ionian Islands</option>
                            <option value="Khuzestan">Khuzestan</option>
                            <option value="Valencia">Valencia</option>
                            <option value="Adan">Adan</option>
                            <option value="Biscay">Biscay</option>
                            <option value="Rhode Island">Rhode Island</option>
                            <option value="Lerida">Lerida</option>
                            <option value="Vermont">Vermont</option>
                            <option value="Phnom Penh">Phnom Penh</option>
                            <option value="Jerusalem">Jerusalem</option>
                            <option value="Opole">Opole</option>
                            <option value="Penghu">Penghu</option>
                            <option value="New South Wales">New South Wales</option>
                            <option value="Cairo">Cairo</option>
                            <option value="Fars">Fars</option>
                            <option value="Panama">Panama</option>
                            <option value="Gaza Strip">Gaza Strip</option>
                            <option value="Vienna">Vienna</option>
                            <option value="North Brabant">North Brabant</option>
                            <option value="Hamburg">Hamburg</option>
                            <option value="New Hampshire">New Hampshire</option>
                            <option value="Delhi">Delhi</option>
                            <option value="Ordu">Ordu</option>
                            <option value="Isabela">Isabela</option>
                            <option value="Ontario">Ontario</option>
                            <option value="Paris">Paris</option>
                            <option value="Central">Central</option>
                            <option value="Aragua">Aragua</option>
                            <option value="Pleiku">Pleiku</option>
                            <option value="Bogota">Bogota</option>
                            <option value="Friuli-Venezia Giulia">Friuli-Venezia Giulia</option>
                            <option value="Queensland">Queensland</option>
                            <option value="Ile-de-France">Ile-de-France</option>
                            <option value="Cavan">Cavan</option>
                            <option value="Kinshasa">Kinshasa</option>
                            <option value="Brazzaville">Brazzaville</option>
                            <option value="Lisbon">Lisbon</option>
                            <option value="North Rhine-Westphalia">North Rhine-Westphalia</option>
                            <option value="Algiers">Algiers</option>
                            <option value="Virginia">Virginia</option>
                            <option value="Zaragoza">Zaragoza</option>
                            <option value="San Salvador">San Salvador</option>
                            <option value="Antioquia">Antioquia</option>
                            <option value="Bangkok">Bangkok</option>
                            <option value="Donegal">Donegal</option>
                            <option value="Santiago">Santiago</option>
                            <option value="Rhineland-Palatinate">Rhineland-Palatinate</option>
                            <option value="Tuscany">Tuscany</option>
                            <option value="Ouest">Ouest</option>
                            <option value="Eritrea">Eritrea</option>
                            <option value="East Azerbaijan">East Azerbaijan</option>
                            <option value="South Dakota">South Dakota</option>
                            <option value="Khartoum">Khartoum</option>
                            <option value="Grand Casablanca">Grand Casablanca</option>
                            <option value="Nicosia">Nicosia</option>
                            <option value="Liguria">Liguria</option>
                            <option value="South">South</option>
                            <option value="An Nabatiyah">An Nabatiyah</option>
                            <option value="Kanagawa">Kanagawa</option>
                            <option value="Kabul">Kabul</option>
                            <option value="Jalisco">Jalisco</option>
                            <option value="Trujillo">Trujillo</option>
                            <option value="Crete">Crete</option>
                            <option value="Risaralda">Risaralda</option>
                            <option value="Lima">Lima</option>
                            <option value="Chaco">Chaco</option>
                            <option value="Santiago Metropolitan">Santiago Metropolitan</option>
                            <option value="Francisco Morazan">Francisco Morazan</option>
                            <option value="Essonne">Essonne</option>
                            <option value="Serbia">Serbia</option>
                            <option value="Schleswig-Holstein">Schleswig-Holstein</option>
                            <option value="Guayas">Guayas</option>
                            <option value="Canterbury">Canterbury</option>
                            <option value="Tokyo">Tokyo</option>
                            <option value="Emilia-Romagna">Emilia-Romagna</option>
                            <option value="Lusaka">Lusaka</option>
                            <option value="Lower Saxony">Lower Saxony</option>
                            <option value="Hauts-de-Seine">Hauts-de-Seine</option>
                            <option value="Kuala Lumpur">Kuala Lumpur</option>
                            <option value="Zulia">Zulia</option>
                            <option value="Doubs">Doubs</option>
                            <option value="Bouches-du-Rhone">Bouches-du-Rhone</option>
                            <option value="Piedmont">Piedmont</option>
                            <option value="Cochabamba">Cochabamba</option>
                            <option value="Mendoza">Mendoza</option>
                            <option value="Saone-et-Loire">Saone-et-Loire</option>
                            <option value="Seine-Saint-Denis">Seine-Saint-Denis</option>
                            <option value="Singapore">Singapore</option>
                            <option value="Santa Ana">Santa Ana</option>
                            <option value="South-East">South-East</option>
                            <option value="Asimah">Asimah</option>
                            <option value="Finistere">Finistere</option>
                            <option value="Haute-Garonne">Haute-Garonne</option>
                            <option value="Oaxaca">Oaxaca</option>
                            <option value="Monaghan">Monaghan</option>
                            <option value="Guerrero">Guerrero</option>
                            <option value="Corsica">Corsica</option>
                            <option value="Maekel">Maekel</option>
                            <option value="Northern">Northern</option>
                            <option value="Zambales">Zambales</option>
                            <option value="Tibesti">Tibesti</option>
                            <option value="Pattani">Pattani</option>
                            <option value="Amanat Al Asimah">Amanat Al Asimah</option>
                            <option value="Yucatan">Yucatan</option>
                            <option value="Wallonia">Wallonia</option>
                            <option value="Flanders">Flanders</option>
                            <option value="Bern">Bern</option>
                            <option value="Macedonia">Macedonia</option>
                            <option value="Hautes-Pyrenees">Hautes-Pyrenees</option>
                            <option value="Hyogo">Hyogo</option>
                            <option value="Gard">Gard</option>
                            <option value="Kingston Parish">Kingston Parish</option>
                            <option value="Aleppo">Aleppo</option>
                            <option value="Salta">Salta</option>
                            <option value="Arequipa">Arequipa</option>
                            <option value="Campania">Campania</option>
                            <option value="Pichincha">Pichincha</option>
                            <option value="Morelos">Morelos</option>
                            <option value="Dubai">Dubai</option>
                            <option value="Lugo">Lugo</option>
                            <option value="Bremen">Bremen</option>
                            <option value="North-West Frontier Province">North-West Frontier Province</option>
                            <option value="Tel Aviv">Tel Aviv</option>
                            <option value="Gauteng">Gauteng</option>
                            <option value="Punjab">Punjab</option>
                            <option value="Vaud">Vaud</option>
                            <option value="Razavi Khorasan">Razavi Khorasan</option>
                            <option value="Bihar">Bihar</option>
                            <option value="State of Mexico">State of Mexico</option>
                            <option value="Nairobi">Nairobi</option>
                            <option value="Cadiz">Cadiz</option>
                            <option value="Finisere">Finisere</option>
                            <option value="Cauca">Cauca</option>
                            <option value="Limassol">Limassol</option>
                            <option value="Meta">Meta</option>
                            <option value="Nineveh">Nineveh</option>
                            <option value="Rhone">Rhone</option>
                            <option value="Galicia">Galicia</option>
                            <option value="Banaadir">Banaadir</option>
                            <option value="Boyaca">Boyaca</option>
                            <option value="Guipuzcoa">Guipuzcoa</option>
                            <option value="Valladolid">Valladolid</option>
                            <option value="Coast">Coast</option>
                            <option value="Mount Lebanon">Mount Lebanon</option>
                            <option value="Haut-Rhin">Haut-Rhin</option>
                            <option value="Pyrenees-Atlantiques">Pyrenees-Atlantiques</option>
                            <option value="Alpes-Maritimes">Alpes-Maritimes</option>
                            <option value="Aichi">Aichi</option>
                            <option value="Seine-Maritime">Seine-Maritime</option>
                            <option value="Seville">Seville</option>
                            <option value="Louth">Louth</option>
                            <option value="Cork">Cork</option>
                            <option value="Pontevedra">Pontevedra</option>
                            <option value="Kildare">Kildare</option>
                            <option value="Okinawa">Okinawa</option>
                            <option value="Zamboanga del Sur">Zamboanga del Sur</option>
                            <option value="Nord">Nord</option>
                            <option value="Alava">Alava</option>
                            <option value="Oshana">Oshana</option>
                            <option value="Casanare">Casanare</option>
                            <option value="Ille-et-Vilaine">Ille-et-Vilaine</option>
                            <option value="Scotland">Scotland</option>
                            <option value="Salamanca">Salamanca</option>
                            <option value="Maine">Maine</option>
                            <option value="Formosa">Formosa</option>
                            <option value="Chiquimula">Chiquimula</option>
                            <option value="Basilan">Basilan</option>
                            <option value="Drenthe">Drenthe</option>
                            <option value="Calabria">Calabria</option>
                            <option value="Anzoategui">Anzoategui</option>
                            <option value="Bimini">Bimini</option>
                            <option value="Lagos">Lagos</option>
                            <option value="Otjozondjupa">Otjozondjupa</option>
                            <option value="Bas-Rhin">Bas-Rhin</option>
                            <option value="Ifugao">Ifugao</option>
                            <option value="Kentucky">Kentucky</option>
                            <option value="Porto">Porto</option>
                            <option value="Atlantico">Atlantico</option>
                            <option value="Misamis Oriental">Misamis Oriental</option>
                            <option value="Veneto">Veneto</option>
                            <option value="Apulia">Apulia</option>
                            <option value="Sardinia">Sardinia</option>
                            <option value="Khomas">Khomas</option>
                            <option value="Trentino-South Tyrol">Trentino-South Tyrol</option>
                            <option value="West Bank">West Bank</option>
                            <option value="Tigray">Tigray</option>
                            <option value="Valle del Cauca">Valle del Cauca</option>
                            <option value="Charente">Charente</option>
                            <option value="Davao del Sur">Davao del Sur</option>
                            <option value="Santander">Santander</option>
                            <option value="Las Palmas">Las Palmas</option>
                            <option value="Sinaloa">Sinaloa</option>
                            <option value="Sonsonate">Sonsonate</option>
                            <option value="Puy-de-Dome">Puy-de-Dome</option>
                            <option value="Saint Michael">Saint Michael</option>
                            <option value="Umbria">Umbria</option>
                            <option value="A Coruna">A Coruna</option>
                            <option value="Isere">Isere</option>
                            <option value="Cantabria">Cantabria</option>
                            <option value="Girona">Girona</option>
                            <option value="San Miguel">San Miguel</option>
                            <option value="Herault">Herault</option>
                            <option value="Port of Spain">Port of Spain</option>
                            <option value="Corse-du-Sud">Corse-du-Sud</option>
                            <option value="Sicily">Sicily</option>
                            <option value="Laoighis">Laoighis</option>
                            <option value="Addis Ababa">Addis Ababa</option>
                            <option value="Usulutan">Usulutan</option>
                            <option value="Christ Church">Christ Church</option>
                            <option value="Islamabad Capital Territory">Islamabad Capital Territory</option>
                            <option value="Marche">Marche</option>
                            <option value="Taipei">Taipei</option>
                            <option value="Kirklareli">Kirklareli</option>
                            <option value="Balikesir">Balikesir</option>
                            <option value="Gironde">Gironde</option>
                            <option value="Rangoon">Rangoon</option>
                            <option value="Transvaal">Transvaal</option>
                            <option value="Chihuahua">Chihuahua</option>
                            <option value="Baghdad">Baghdad</option>
                            <option value="Basel-Stadt">Basel-Stadt</option>
                            <option value="Chittagong">Chittagong</option>
                            <option value="Cabinda">Cabinda</option>
                            <option value="Caqueta">Caqueta</option>
                            <option value="Konya">Konya</option>
                            <option value="Central Macedonia">Central Macedonia</option>
                            <option value="Leon">Leon</option>
                            <option value="Adana">Adana</option>
                            <option value="Osmaniye">Osmaniye</option>
                            <option value="Diyarbakir">Diyarbakir</option>
                            <option value="Zinguldak">Zinguldak</option>
                            <option value="Gaziantep">Gaziantep</option>
                            <option value="Bursa">Bursa</option>
                            <option value="Azores">Azores</option>
                            <option value="Antalya">Antalya</option>
                            <option value="Aydin">Aydin</option>
                            <option value="Manisa">Manisa</option>
                            <option value="Somali">Somali</option>
                            <option value="Katanga">Katanga</option>
                            <option value="Alajuela">Alajuela</option>
                            <option value="Tiris Zemmour">Tiris Zemmour</option>
                            <option value="Mersin">Mersin</option>
                            <option value="Erzincan">Erzincan</option>
                            <option value="Songkhla">Songkhla</option>
                            <option value="Ourense">Ourense</option>
                            <option value="Toledo">Toledo</option>
                            <option value="Kocaeli">Kocaeli</option>
                            <option value="Seine-et-Marne">Seine-et-Marne</option>
                            <option value="Slovenia">Slovenia</option>
                            <option value="Tarragona">Tarragona</option>
                            <option value="Arica and Parinacota">Arica and Parinacota</option>
                            <option value="Sivas">Sivas</option>
                            <option value="Cotes-d\Armor">Cotes-d\Armor</option>
                            <option value="San Luis Potosi">San Luis Potosi</option>
                            <option value="KwaZulu-Natal">KwaZulu-Natal</option>
                            <option value="Samsun">Samsun</option>
                            <option value="Southern">Southern</option>
                            <option value="Hatay">Hatay</option>
                            <option value="Carinthia">Carinthia</option>
                            <option value="Malaga">Malaga</option>
                            <option value="Sanliurfa">Sanliurfa</option>
                            <option value="Tokat">Tokat</option>
                            <option value="Kahramanmaras">Kahramanmaras</option>
                            <option value="Erzurum">Erzurum</option>
                            <option value="Bolivar">Bolivar</option>
                            <option value="Evora">Evora</option>
                            <option value="Santarem">Santarem</option>
                            <option value="Setubal">Setubal</option>
                            <option value="Beja">Beja</option>
                            <option value="Tolima">Tolima</option>
                            <option value="Utrecht">Utrecht</option>
                            <option value="Yala">Yala</option>
                            <option value="Maharashtra">Maharashtra</option>
                            <option value="Haute-Corse">Haute-Corse</option>
                            <option value="Nueva Segovia">Nueva Segovia</option>
                            <option value="Balearic Islands">Balearic Islands</option>
                            <option value="Rio San Juan">Rio San Juan</option>
                            <option value="Abruzzo">Abruzzo</option>
                            <option value="Masaya">Masaya</option>
                            <option value="Cundinamarca">Cundinamarca</option>
                            <option value="Mayenne">Mayenne</option>
                            <option value="Abu Dhabi">Abu Dhabi</option>
                            <option value="Segovia">Segovia</option>
                            <option value="Dikhil">Dikhil</option>
                            <option value="North Sumatra">North Sumatra</option>
                            <option value="Huila">Huila</option>
                            <option value="Moselle">Moselle</option>
                            <option value="Jura">Jura</option>
                            <option value="Putumayo">Putumayo</option>
                            <option value="Quindio">Quindio</option>
                            <option value="Murcia">Murcia</option>
                            <option value="Yonne">Yonne</option>
                            <option value="Aude">Aude</option>
                            <option value="Kyoto">Kyoto</option>
                            <option value="Mashonaland South">Mashonaland South</option>
                            <option value="Russia">Russia</option>
                            <option value="Basilicata">Basilicata</option>
                            <option value="Maguindanao">Maguindanao</option>
                            <option value="Chinandega">Chinandega</option>
                            <option value="Ibaraki">Ibaraki</option>
                            <option value="Matagalpa">Matagalpa</option>
                            <option value="Granada">Granada</option>
                            <option value="Palencia">Palencia</option>
                            <option value="Esteli">Esteli</option>
                            <option value="Alaska">Alaska</option>
                            <option value="Carabobo">Carabobo</option>
                            <option value="Sucre">Sucre</option>
                            <option value="Chiba">Chiba</option>
                            <option value="Carazo">Carazo</option>
                            <option value="Cabanas">Cabanas</option>
                            <option value="San Vicente">San Vicente</option>
                            <option value="Asturias">Asturias</option>
                            <option value="North West">North West</option>
                            <option value="Eastern Cape">Eastern Cape</option>
                            <option value="Haifa">Haifa</option>
                            <option value="Cuscatlan">Cuscatlan</option>
                            <option value="Chalatenango">Chalatenango</option>
                            <option value="Parana">Parana</option>
                            <option value="Tamaulipas">Tamaulipas</option>
                            <option value="San Pedro de Macoris">San Pedro de Macoris</option>
                            <option value="Papua">Papua</option>
                            <option value="Saitama">Saitama</option>
                            <option value="Manicaland">Manicaland</option>
                            <option value="Cesar">Cesar</option>
                            <option value="Qom">Qom</option>
                            <option value="Norte de Santander">Norte de Santander</option>
                            <option value="La Guajira">La Guajira</option>
                            <option value="Tribal Trust Lands">Tribal Trust Lands</option>
                            <option value="Escuintla">Escuintla</option>
                            <option value="Doha">Doha</option>
                            <option value="Magdalena">Magdalena</option>
                            <option value="Matabeleland South">Matabeleland South</option>
                            <option value="Limerick">Limerick</option>
                            <option value="North">North</option>
                            <option value="Osaka">Osaka</option>
                            <option value="Midland">Midland</option>
                            <option value="Yvelines">Yvelines</option>
                            <option value="Mashonaland West">Mashonaland West</option>
                            <option value="Huehuetenango">Huehuetenango</option>
                            <option value="Aosta">Aosta</option>
                            <option value="Sulu">Sulu</option>
                            <option value="Ayacucho">Ayacucho</option>
                            <option value="Soria">Soria</option>
                            <option value="Sa Kaeo">Sa Kaeo</option>
                            <option value="Mashonland">Mashonland</option>
                            <option value="Lanao del Sur">Lanao del Sur</option>
                            <option value="Quetzaltenango">Quetzaltenango</option>
                            <option value="North Cotabato">North Cotabato</option>
                            <option value="Minas Gerais">Minas Gerais</option>
                            <option value="Salisbury">Salisbury</option>
                            <option value="Davao Oriental">Davao Oriental</option>
                            <option value="Kalinga">Kalinga</option>
                            <option value="Veracruz">Veracruz</option>
                            <option value="Davao del Norte">Davao del Norte</option>
                            <option value="North Sinai">North Sinai</option>
                            <option value="Surat Thani">Surat Thani</option>
                            <option value="Totonicapan">Totonicapan</option>
                            <option value="Daraa">Daraa</option>
                            <option value="Basque Country">Basque Country</option>
                            <option value="Monagas">Monagas</option>
                            <option value="La Libertad">La Libertad</option>
                            <option value="Kars">Kars</option>
                            <option value="Omusati">Omusati</option>
                            <option value="Bingol">Bingol</option>
                            <option value="San Marcos">San Marcos</option>
                            <option value="Chontales">Chontales</option>
                            <option value="Huambo">Huambo</option>
                            <option value="Nueva Ecija">Nueva Ecija</option>
                            <option value="Rivas">Rivas</option>
                            <option value="Ahuachapan">Ahuachapan</option>
                            <option value="Antofagasta">Antofagasta</option>
                            <option value="Barima-Waini">Barima-Waini</option>
                            <option value="Miranda">Miranda</option>
                            <option value="Maputo">Maputo</option>
                            <option value="Andalusia">Andalusia</option>
                            <option value="Delaware">Delaware</option>
                            <option value="Sultan Kudarat">Sultan Kudarat</option>
                            <option value="Mardin">Mardin</option>
                            <option value="Guelmim-Es Semara">Guelmim-Es Semara</option>
                            <option value="Chubut">Chubut</option>
                            <option value="Sofala">Sofala</option>
                            <option value="Narino">Narino</option>
                            <option value="Ica">Ica</option>
                            <option value="Erongo">Erongo</option>
                            <option value="Gwelo">Gwelo</option>
                            <option value="Shan">Shan</option>
                            <option value="Australian Capital Territory">Australian Capital Territory</option>
                            <option value="Canary Islands">Canary Islands</option>
                            <option value="Madriz">Madriz</option>
                            <option value="Tunis">Tunis</option>
                            <option value="Umtali">Umtali</option>
                            <option value="Assam">Assam</option>
                            <option value="Manipur">Manipur</option>
                            <option value="Capital">Capital</option>
                            <option value="Kerman">Kerman</option>
                            <option value="Catalonia">Catalonia</option>
                            <option value="Quiche">Quiche</option>
                            <option value="Bulawayo">Bulawayo</option>
                            <option value="Meknes-Tafilalet">Meknes-Tafilalet</option>
                            <option value="Oran">Oran</option>
                            <option value="Choco">Choco</option>
                            <option value="Eastern">Eastern</option>
                            <option value="Chiriqui">Chiriqui</option>
                            <option value="Puebla">Puebla</option>
                            <option value="Ohangwena">Ohangwena</option>
                            <option value="Melilla">Melilla</option>
                            <option value="Qazvin">Qazvin</option>
                            <option value="Trang">Trang</option>
                            <option value="Aargau">Aargau</option>
                            <option value="Valparaiso">Valparaiso</option>
                            <option value="Oslo">Oslo</option>
                            <option value="West Azerbaijan">West Azerbaijan</option>
                            <option value="Ceuta">Ceuta</option>
                            <option value="Mon">Mon</option>
                            <option value="Lanao del Norte">Lanao del Norte</option>
                            <option value="Oshikoto">Oshikoto</option>
                            <option value="Chin">Chin</option>
                            <option value="Tawi Tawi">Tawi Tawi</option>
                            <option value="Dhaka">Dhaka</option>
                            <option value="Western Australia">Western Australia</option>
                            <option value="Var">Var</option>
                            <option value="Inhambane">Inhambane</option>
                            <option value="Krakow">Krakow</option>
                            <option value="Matabeleland">Matabeleland</option>
                            <option value="Manica">Manica</option>
                            <option value="Krabi">Krabi</option>
                            <option value="Kanchanaburi">Kanchanaburi</option>
                            <option value="Chiang Mai">Chiang Mai</option>
                            <option value="Suchitepequez">Suchitepequez</option>
                            <option value="Zamboanga del Norte">Zamboanga del Norte</option>
                            <option value="Maseru">Maseru</option>
                            <option value="Cortes">Cortes</option>
                            <option value="Jinotega">Jinotega</option>
                            <option value="Western Cape">Western Cape</option>
                            <option value="Castile and Leon">Castile and Leon</option>
                            <option value="South Caribbean Coast Autonomous Region">South Caribbean Coast Autonomous Region</option>
                            <option value="Zacapa">Zacapa</option>
                            <option value="Luanda">Luanda</option>
                            <option value="Zelaya">Zelaya</option>
                            <option value="Udon Thani">Udon Thani</option>
                            <option value="Hamah">Hamah</option>
                            <option value="Kermanshah">Kermanshah</option>
                            <option value="Barinas">Barinas</option>
                            <option value="Northern Samar">Northern Samar</option>
                            <option value="Dar es Salaam">Dar es Salaam</option>
                            <option value="Mizoram">Mizoram</option>
                            <option value="Sistan and Balochistan">Sistan and Balochistan</option>
                            <option value="Basra">Basra</option>
                            <option value="Mashonaland East">Mashonaland East</option>
                            <option value="Estuaire">Estuaire</option>
                            <option value="Bio Bio">Bio Bio</option>
                            <option value="Aragon">Aragon</option>
                            <option value="Rif Dimashq">Rif Dimashq</option>
                            <option value="Damascus">Damascus</option>
                            <option value="Val-de-Marne">Val-de-Marne</option>
                            <option value="Burdur">Burdur</option>
                            <option value="Ilam">Ilam</option>
                            <option value="Golan Heights">Golan Heights</option>
                            <option value="Demerara-Mahaica">Demerara-Mahaica</option>
                            <option value="Kordestan">Kordestan</option>
                            <option value="Perak">Perak</option>
                            <option value="Denizli">Denizli</option>
                            <option value="Waterford">Waterford</option>
                            <option value="Homs">Homs</option>
                            <option value="Andhra Pradesh">Andhra Pradesh</option>
                            <option value="Coquimbo">Coquimbo</option>
                            <option value="Zanjan">Zanjan</option>
                            <option value="Latakia">Latakia</option>
                            <option value="Tripoli">Tripoli</option>
                            <option value="Jharkhand">Jharkhand</option>
                            <option value="Sligo">Sligo</option>
                            <option value="Ghazni">Ghazni</option>
                            <option value="Tunceli">Tunceli</option>
                            <option value="Tamil Nadu">Tamil Nadu</option>
                            <option value="Herat">Herat</option>
                            <option value="Choluteca">Choluteca</option>
                            <option value="Bitlis">Bitlis</option>
                            <option value="Espirito Santo">Espirito Santo</option>
                            <option value="Coimbra">Coimbra</option>
                            <option value="La Union">La Union</option>
                            <option value="Santa Cruz">Santa Cruz</option>
                            <option value="Murqub">Murqub</option>
                            <option value="Adiyaman">Adiyaman</option>
                            <option value="Apure">Apure</option>
                            <option value="Solola">Solola</option>
                            <option value="Peten">Peten</option>
                            <option value="Solothurn">Solothurn</option>
                            <option value="Mpumalanga">Mpumalanga</option>
                            <option value="Makkah">Makkah</option>
                            <option value="Kayseri">Kayseri</option>
                            <option value="Alta Verapaz">Alta Verapaz</option>
                            <option value="Uttar Pradesh">Uttar Pradesh</option>
                            <option value="Oruro">Oruro</option>
                            <option value="Kaduna">Kaduna</option>
                            <option value="Espaillat">Espaillat</option>
                            <option value="Gafsa">Gafsa</option>
                            <option value="Orange Free State">Orange Free State</option>
                            <option value="Nong Khai">Nong Khai</option>
                            <option value="Hong Kong">Hong Kong</option>
                            <option value="Natal">Natal</option>
                            <option value="Midlands">Midlands</option>
                            <option value="El Progreso">El Progreso</option>
                            <option value="Caaguazu">Caaguazu</option>
                            <option value="Misamis Occidental">Misamis Occidental</option>
                            <option value="Baja Verapaz">Baja Verapaz</option>
                            <option value="Bophutatswana">Bophutatswana</option>
                            <option value="Morazán">Morazán</option>
                            <option value="Dakar">Dakar</option>
                            <option value="Nakhon Si Thammarat">Nakhon Si Thammarat</option>
                            <option value="At Tamim">At Tamim</option>
                            <option value="Kamphaeng Phet">Kamphaeng Phet</option>
                            <option value="West Bengal">West Bengal</option>
                            <option value="Arauca">Arauca</option>
                            <option value="Lorestan">Lorestan</option>
                            <option value="Golestan">Golestan</option>
                            <option value="Izabal">Izabal</option>
                            <option value="Beqaa">Beqaa</option>
                            <option value="Usak">Usak</option>
                            <option value="Narathiwat">Narathiwat</option>
                            <option value="Elazig">Elazig</option>
                            <option value="Chaiyaphum">Chaiyaphum</option>
                            <option value="Jahra">Jahra</option>
                            <option value="Jutiapa">Jutiapa</option>
                            <option value="Trabzon">Trabzon</option>
                            <option value="Algarve">Algarve</option>
                            <option value="Conakry">Conakry</option>
                            <option value="Chimaltenango">Chimaltenango</option>
                            <option value="Boaco">Boaco</option>
                            <option value="Amasya">Amasya</option>
                            <option value="Al Hasakah">Al Hasakah</option>
                            <option value="Nevsehir">Nevsehir</option>
                            <option value="Tarija">Tarija</option>
                            <option value="Saint George">Saint George</option>
                            <option value="Potosi">Potosi</option>
                            <option value="Offaly">Offaly</option>
                            <option value="Shefa">Shefa</option>
                            <option value="Belize">Belize</option>
                            <option value="Siirt">Siirt</option>
                            <option value="Ismailia">Ismailia</option>
                            <option value="Wales">Wales</option>
                            <option value="La Rioja">La Rioja</option>
                            <option value="Benghazi">Benghazi</option>
                            <option value="Hamadan">Hamadan</option>
                            <option value="Sacatepequez">Sacatepequez</option>
                            <option value="El Paraiso">El Paraiso</option>
                            <option value="Ancash">Ancash</option>
                            <option value="Junin">Junin</option>
                            <option value="Retahuleu">Retahuleu</option>
                            <option value="Cusco">Cusco</option>
                            <option value="Benguet">Benguet</option>
                            <option value="Huancavelica">Huancavelica</option>
                            <option value="Santa Rosa">Santa Rosa</option>
                            <option value="Pointe-a-Pitre">Pointe-a-Pitre</option>
                            <option value="Samar">Samar</option>
                            <option value="Cavite">Cavite</option>
                            <option value="Cabrican">Cabrican</option>
                            <option value="Atacama">Atacama</option>
                            <option value="Lambayeque">Lambayeque</option>
                            <option value="Wexford">Wexford</option>
                            <option value="Pasco">Pasco</option>
                            <option value="Guainia">Guainia</option>
                            <option value="Saint Patrick">Saint Patrick</option>
                            <option value="Central Jutland">Central Jutland</option>
                            <option value="Callao">Callao</option>
                            <option value="Santa Barbara">Santa Barbara</option>
                            <option value="Puno">Puno</option>
                            <option value="Loreto">Loreto</option>
                            <option value="South Cotabato">South Cotabato</option>
                            <option value="Valle">Valle</option>
                            <option value="Sulaymaniyah">Sulaymaniyah</option>
                            <option value="North Caribbean Coast Autonomous Region">North Caribbean Coast Autonomous Region</option>
                            <option value="Idlib">Idlib</option>
                            <option value="Masvingo">Masvingo</option>
                            <option value="Jakarta">Jakarta</option>
                            <option value="New Providence">New Providence</option>
                            <option value="Cajamarca">Cajamarca</option>
                            <option value="Nagaland">Nagaland</option>
                            <option value="El Peten">El Peten</option>
                            <option value="Moquegua">Moquegua</option>
                            <option value="Limpopo">Limpopo</option>
                            <option value="Vatican City">Vatican City</option>
                            <option value="Mato Grosso do Sul">Mato Grosso do Sul</option>
                            <option value="Kirkuk">Kirkuk</option>
                            <option value="Eastern Samar">Eastern Samar</option>
                            <option value="Guna Yala">Guna Yala</option>
                            <option value="Bangui">Bangui</option>
                            <option value="Tarapaca">Tarapaca</option>
                            <option value="Kohgiluyeh and Buyer Ahmad">Kohgiluyeh and Buyer Ahmad</option>
                            <option value="Mazandaran">Mazandaran</option>
                            <option value="Warsaw">Warsaw</option>
                            <option value="Brandenburg">Brandenburg</option>
                            <option value="Sucumbios">Sucumbios</option>
                            <option value="Yazd">Yazd</option>
                            <option value="Tacna">Tacna</option>
                            <option value="Gujarat">Gujarat</option>
                            <option value="Los Rios">Los Rios</option>
                            <option value="Chiang Rai">Chiang Rai</option>
                            <option value="Asyut">Asyut</option>
                            <option value="Chandigarh">Chandigarh</option>
                            <option value="Amazonas">Amazonas</option>
                            <option value="Castelo Branco">Castelo Branco</option>
                            <option value="Loire-Atlantique">Loire-Atlantique</option>
                            <option value="Lot-et-Garonne">Lot-et-Garonne</option>
                            <option value="Lempira">Lempira</option>
                            <option value="Mont Buxton">Mont Buxton</option>
                            <option value="Apurimac">Apurimac</option>
                            <option value="Western">Western</option>
                            <option value="Sirnak">Sirnak</option>
                            <option value="Huanuco">Huanuco</option>
                            <option value="Aquitaine">Aquitaine</option>
                            <option value="Atlantico Norte">Atlantico Norte</option>
                            <option value="Rhone-Alpes">Rhone-Alpes</option>
                            <option value="Cartago">Cartago</option>
                            <option value="Provence-Alpes-Cote d\Azur">Provence-Alpes-Cote d\Azur</option>
                            <option value="Abra">Abra</option>
                            <option value="Chiapas">Chiapas</option>
                            <option value="Busan">Busan</option>
                            <option value="Khulna">Khulna</option>
                            <option value="Retalhuleu">Retalhuleu</option>
                            <option value="Limousin">Limousin</option>
                            <option value="Quneitra">Quneitra</option>
                            <option value="Chimborazo">Chimborazo</option>
                            <option value="Languedoc-Roussillon">Languedoc-Roussillon</option>
                            <option value="Midi-Pyrenees">Midi-Pyrenees</option>
                            <option value="Juitapa">Juitapa</option>
                            <option value="Matabeleland North">Matabeleland North</option>
                            <option value="Salzburg">Salzburg</option>
                            <option value="Rajasthan">Rajasthan</option>
                            <option value="Burgas">Burgas</option>
                            <option value="Paramaribo">Paramaribo</option>
                            <option value="Cuanza Sul">Cuanza Sul</option>
                            <option value="Manzini">Manzini</option>
                            <option value="Zambezia">Zambezia</option>
                            <option value="Bukidnon">Bukidnon</option>
                            <option value="Piura">Piura</option>
                            <option value="Neuquén">Neuquén</option>
                            <option value="Tete">Tete</option>
                            <option value="Cuscaltan">Cuscaltan</option>
                            <option value="North Dakota">North Dakota</option>
                            <option value="Benguela">Benguela</option>
                            <option value="Tanintharyi">Tanintharyi</option>
                            <option value="Gaza">Gaza</option>
                            <option value="Tachira">Tachira</option>
                            <option value="Luxembourg">Luxembourg</option>
                            <option value="Guaviare">Guaviare</option>
                            <option value="Berea">Berea</option>
                            <option value="Quthing">Quthing</option>
                            <option value="Leribe">Leribe</option>
                            <option value="Chhattisgarh">Chhattisgarh</option>
                            <option value="Yoro">Yoro</option>
                            <option value="Eastern Macedonia and Thrace">Eastern Macedonia and Thrace</option>
                            <option value="Faro">Faro</option>
                            <option value="Capital Region">Capital Region</option>
                            <option value="Chuquisaca">Chuquisaca</option>
                            <option value="Saint Laurent Du Maroni">Saint Laurent Du Maroni</option>
                            <option value="Cayenne">Cayenne</option>
                            <option value="Fort-de-France">Fort-de-France</option>
                            <option value="O\Higgins">O\Higgins</option>
                            <option value="Butha-Buthe">Butha-Buthe</option>
                            <option value="Malanje">Malanje</option>
                            <option value="Maule">Maule</option>
                            <option value="East Falkland">East Falkland</option>
                            <option value="Bahia">Bahia</option>
                            <option value="Ciskei">Ciskei</option>
                            <option value="Lunda Norte">Lunda Norte</option>
                            <option value="Olancho">Olancho</option>
                            <option value="San Martin">San Martin</option>
                            <option value="Atlantida">Atlantida</option>
                            <option value="Catamarca">Catamarca</option>
                            <option value="Para">Para</option>
                            <option value="Daegu">Daegu</option>
                            <option value="Zarqa">Zarqa</option>
                            <option value="Michoacan">Michoacan</option>
                            <option value="Yamaguchi">Yamaguchi</option>
                            <option value="Arkansas">Arkansas</option>
                            <option value="Falcon">Falcon</option>
                            <option value="Atlantico Sur">Atlantico Sur</option>
                            <option value="Hormozgan">Hormozgan</option>
                            <option value="Basse-Terre">Basse-Terre</option>
                            <option value="Al Wahdah">Al Wahdah</option>
                            <option value="Leiria">Leiria</option>
                            <option value="Ahmadi">Ahmadi</option>
                            <option value="Leitrim">Leitrim</option>
                            <option value="Tripura">Tripura</option>
                            <option value="West Kordofan">West Kordofan</option>
                            <option value="Haryana">Haryana</option>
                            <option value="Napo">Napo</option>
                            <option value="N\Djamena">N\Djamena</option>
                            <option value="Los Lagos">Los Lagos</option>
                            <option value="Alsace">Alsace</option>
                            <option value="Wellington">Wellington</option>
                            <option value="Le Marin">Le Marin</option>
                            <option value="Magallanes & Chilean Antarctica">Magallanes & Chilean Antarctica</option>
                            <option value="Camarines Sur">Camarines Sur</option>
                            <option value="Negros Occidental">Negros Occidental</option>
                            <option value="Central Equatoria">Central Equatoria</option>
                            <option value="Jammu and Kashmir">Jammu and Kashmir</option>
                            <option value="Federal District">Federal District</option>
                            <option value="Himachal Pradesh">Himachal Pradesh</option>
                            <option value="Jabal al Akhdar">Jabal al Akhdar</option>
                            <option value="Colon">Colon</option>
                            <option value="Laayoune-Boujdour-Sakia El Hamra">Laayoune-Boujdour-Sakia El Hamra</option>
                            <option value="Magdeburg">Magdeburg</option>
                            <option value="Kavango">Kavango</option>
                            <option value="Kosovo">Kosovo</option>
                            <option value="Araucania">Araucania</option>
                            <option value="Centre">Centre</option>
                            <option value="Woqooyi Galbeed">Woqooyi Galbeed</option>
                            <option value="Federally Administered Tribal Areas">Federally Administered Tribal Areas</option>
                            <option value="Karnataka">Karnataka</option>
                            <option value="Hakkari">Hakkari</option>
                            <option value="Varna">Varna</option>
                            <option value="Larnaca">Larnaca</option>
                            <option value="Cuanza Norte">Cuanza Norte</option>
                            <option value="South Eastern">South Eastern</option>
                            <option value="Surigao del Sur">Surigao del Sur</option>
                            <option value="South Province">South Province</option>
                            <option value="North Province">North Province</option>
                            <option value="Bengo">Bengo</option>
                            <option value="Davao">Davao</option>
                            <option value="Bucharest">Bucharest</option>
                            <option value="Hhohho">Hhohho</option>
                            <option value="Cebu">Cebu</option>
                            <option value="Uige">Uige</option>
                            <option value="Aguasan Del Sur">Aguasan Del Sur</option>
                            <option value="Saxony-Anhalt">Saxony-Anhalt</option>
                            <option value="Chiaha">Chiaha</option>
                            <option value="East Java">East Java</option>
                            <option value="Riyadh">Riyadh</option>
                            <option value="Valais">Valais</option>
                            <option value="Center">Center</option>
                            <option value="San Juan">San Juan</option>
                            <option value="Ilocos Norte">Ilocos Norte</option>
                            <option value="Esmeraldas">Esmeraldas</option>
                            <option value="Manabi">Manabi</option>
                            <option value="North Central">North Central</option>
                            <option value="Rabat-Sale-Zemmour-Zaer">Rabat-Sale-Zemmour-Zaer</option>
                            <option value="Tahoua">Tahoua</option>
                            <option value="Agusan del Norte">Agusan del Norte</option>
                            <option value="Brittany">Brittany</option>
                            <option value="Apayao">Apayao</option>
                            <option value="Guanacaste">Guanacaste</option>
                            <option value="Far-Western">Far-Western</option>
                            <option value="North Western">North Western</option>
                            <option value="Cagayan">Cagayan</option>
                            <option value="Bataan">Bataan</option>
                            <option value="Vichada">Vichada</option>
                            <option value="Saarland">Saarland</option>
                            <option value="Ucayali">Ucayali</option>
                            <option value="Caprivi">Caprivi</option>
                            <option value="Iloilo">Iloilo</option>
                            <option value="Zamboanga Sibugay">Zamboanga Sibugay</option>
                            <option value="Kalinga Apayao">Kalinga Apayao</option>
                            <option value="Entre Rios">Entre Rios</option>
                            <option value="Maritime">Maritime</option>
                            <option value="Bulacan">Bulacan</option>
                            <option value="Nueva Vizcaya">Nueva Vizcaya</option>
                            <option value="Groningen">Groningen</option>
                            <option value="Leyte">Leyte</option>
                            <option value="Norhtern">Norhtern</option>
                            <option value="Uva">Uva</option>
                            <option value="Sabah">Sabah</option>
                            <option value="Oriental Mindoro">Oriental Mindoro</option>
                            <option value="Camiguin">Camiguin</option>
                            <option value="Negros Oriental">Negros Oriental</option>
                            <option value="Tyrol">Tyrol</option>
                            <option value="Yaracuy">Yaracuy</option>
                            <option value="Jujuy">Jujuy</option>
                            <option value="North-Western">North-Western</option>
                            <option value="San Luis">San Luis</option>
                            <option value="British Columbia">British Columbia</option>
                            <option value="Bie">Bie</option>
                            <option value="Eastern Equatoria">Eastern Equatoria</option>
                            <option value="Balochistan">Balochistan</option>
                            <option value="Uusimaa">Uusimaa</option>
                            <option value="Azuay">Azuay</option>
                            <option value="Sabaragamuwa">Sabaragamuwa</option>
                            <option value="Marowijne">Marowijne</option>
                            <option value="KwaNdebele">KwaNdebele</option>
                            <option value="Northern Cape">Northern Cape</option>
                            <option value="Maldonado">Maldonado</option>
                            <option value="Extremadura">Extremadura</option>
                            <option value="Seoul">Seoul</option>
                            <option value="Acre">Acre</option>
                            <option value="Castile-La Mancha">Castile-La Mancha</option>
                            <option value="Western Region">Western Region</option>
                            <option value="Batman">Batman</option>
                            <option value="Guarico">Guarico</option>
                            <option value="Upper Nile">Upper Nile</option>
                            <option value="Ocotepeque">Ocotepeque</option>
                            <option value="Djibouti">Djibouti</option>
                            <option value="Commewyne">Commewyne</option>
                            <option value="Jalapa">Jalapa</option>
                            <option value="Quezon">Quezon</option>
                            <option value="Brokopondo">Brokopondo</option>
                            <option value="Harare">Harare</option>
                            <option value="Merida">Merida</option>
                            <option value="Dihok">Dihok</option>
                            <option value="Ilocos Sur">Ilocos Sur</option>
                            <option value="Tumbes">Tumbes</option>
                            <option value="Agri">Agri</option>
                            <option value="Khyber Pakhtunkhwa">Khyber Pakhtunkhwa</option>
                            <option value="Guimaras">Guimaras</option>
                            <option value="Cuando Cubango">Cuando Cubango</option>
                            <option value="Greater Accra">Greater Accra</option>
                            <option value="Karas">Karas</option>
                            <option value="Batangas">Batangas</option>
                            <option value="Albay">Albay</option>
                            <option value="Rewa">Rewa</option>
                            <option value="Sousse">Sousse</option>
                            <option value="Monastir">Monastir</option>
                            <option value="Comayagua">Comayagua</option>
                            <option value="Misiones">Misiones</option>
                            <option value="Uttaranchal">Uttaranchal</option>
                            <option value="Madhya Pradesh">Madhya Pradesh</option>
                            <option value="Saladin">Saladin</option>
                            <option value="Barisal">Barisal</option>
                            <option value="Gumushane">Gumushane</option>
                            <option value="Monsenor Nouel">Monsenor Nouel</option>
                            <option value="Orissa">Orissa</option>
                            <option value="Pangasinan">Pangasinan</option>
                            <option value="Zamora-Chinchipe">Zamora-Chinchipe</option>
                            <option value="Transkei">Transkei</option>
                            <option value="Kachin">Kachin</option>
                            <option value="Cabo Delgado">Cabo Delgado</option>
                            <option value="Sennar">Sennar</option>
                            <option value="Biliran">Biliran</option>
                            <option value="Occidental Mindoro">Occidental Mindoro</option>
                            <option value="Southern Leyte">Southern Leyte</option>
                            <option value="North Eastern">North Eastern</option>
                            <option value="Rajshahi">Rajshahi</option>
                            <option value="Corum">Corum</option>
                            <option value="Eastern Province">Eastern Province</option>
                            <option value="Mountain Province">Mountain Province</option>
                            <option value="Chonburi">Chonburi</option>
                            <option value="Faryab">Faryab</option>
                            <option value="Kayin">Kayin</option>
                            <option value="Vaupes">Vaupes</option>
                            <option value="Limburg">Limburg</option>
                            <option value="Sylhet">Sylhet</option>
                            <option value="Loyalty Islands Province">Loyalty Islands Province</option>
                            <option value="Sorsogon">Sorsogon</option>
                            <option value="Sakarya">Sakarya</option>
                            <option value="Paraiba">Paraiba</option>
                            <option value="Kilis">Kilis</option>
                            <option value="Gwangju">Gwangju</option>
                            <option value="Western Greece">Western Greece</option>
                            <option value="Togdheer">Togdheer</option>
                            <option value="Van">Van</option>
                            <option value="Laguna">Laguna</option>
                            <option value="Nangarhar">Nangarhar</option>
                            <option value="Rangpur">Rangpur</option>
                            <option value="Sohag">Sohag</option>
                            <option value="Orientale">Orientale</option>
                            <option value="Franche-Comte">Franche-Comte</option>
                            <option value="Bohol">Bohol</option>
                            <option value="Male">Male</option>
                            <option value="Plaines Wilhems">Plaines Wilhems</option>
                            <option value="Tabasco">Tabasco</option>
                            <option value="Kayah">Kayah</option>
                            <option value="Nampula">Nampula</option>
                            <option value="Overijssel">Overijssel</option>
                            <option value="South Australia">South Australia</option>
                            <option value="Sipaliwini">Sipaliwini</option>
                            <option value="Ziguinchor">Ziguinchor</option>
                            <option value="Aysen">Aysen</option>
                            <option value="East Timor">East Timor</option>
                            <option value="Ishikawa">Ishikawa</option>
                            <option value="Minya">Minya</option>
                            <option value="Barahona">Barahona</option>
                            <option value="Severocesky">Severocesky</option>
                            <option value="Yangon">Yangon</option>
                            <option value="Budapest">Budapest</option>
                            <option value="Mandalay">Mandalay</option>
                            <option value="Vientiane">Vientiane</option>
                            <option value="La Pampa">La Pampa</option>
                            <option value="Savanne">Savanne</option>
                            <option value="Masbate">Masbate</option>
                            <option value="Camarines Norte">Camarines Norte</option>
                            <option value="Prague">Prague</option>
                            <option value="Nassa">Nassa</option>
                            <option value="Battambang">Battambang</option>
                            <option value="Aklan">Aklan</option>
                            <option value="Croatia">Croatia</option>
                            <option value="Faiyum">Faiyum</option>
                            <option value="Jonglei">Jonglei</option>
                            <option value="Zhejiang">Zhejiang</option>
                            <option value="South Aegean">South Aegean</option>
                            <option value="Jihomoravsky">Jihomoravsky</option>
                            <option value="Pays-de-la-Loire">Pays-de-la-Loire</option>
                            <option value="Central Greece">Central Greece</option>
                            <option value="Kerala">Kerala</option>
                            <option value="Rift Valley">Rift Valley</option>
                            <option value="Hermanas Mirabal">Hermanas Mirabal</option>
                            <option value="Bougainville">Bougainville</option>
                            <option value="Gyeonggi">Gyeonggi</option>
                            <option value="Tungurahua">Tungurahua</option>
                            <option value="Shanghai">Shanghai</option>
                            <option value="Awdal">Awdal</option>
                            <option value="Igdir">Igdir</option>
                            <option value="Nakhon Ratchasima">Nakhon Ratchasima</option>
                            <option value="Lampang">Lampang</option>
                            <option value="Kandahar">Kandahar</option>
                            <option value="Al Sharqia">Al Sharqia</option>
                            <option value="Kunene">Kunene</option>
                            <option value="Bartin">Bartin</option>
                            <option value="Eskisehir">Eskisehir</option>
                            <option value="Armenia">Armenia</option>
                            <option value="Odisha">Odisha</option>
                            <option value="Coronie">Coronie</option>
                            <option value="Morobe">Morobe</option>
                            <option value="North Solomons">North Solomons</option>
                            <option value="North Aegean">North Aegean</option>
                            <option value="Surigao del Norte">Surigao del Norte</option>
                            <option value="Azerbaijan">Azerbaijan</option>
                            <option value="Zinder">Zinder</option>
                            <option value="Aceh">Aceh</option>
                            <option value="Ba">Ba</option>
                            <option value="Saramacca">Saramacca</option>
                            <option value="South Kordofan">South Kordofan</option>
                            <option value="Kastamonu">Kastamonu</option>
                            <option value="Venda">Venda</option>
                            <option value="Loja">Loja</option>
                            <option value="Nara">Nara</option>
                            <option value="Blida">Blida</option>
                            <option value="Nagasaki">Nagasaki</option>
                            <option value="Moxico">Moxico</option>
                            <option value="Azad Kashmir">Azad Kashmir</option>
                            <option value="Tak">Tak</option>
                            <option value="Uppsala">Uppsala</option>
                            <option value="Meath">Meath</option>
                            <option value="Nimba">Nimba</option>
                            <option value="Ubon Ratchathani">Ubon Ratchathani</option>
                            <option value="Mashonaland Central">Mashonaland Central</option>
                            <option value="Kgatleng">Kgatleng</option>
                            <option value="Pointe Noire">Pointe Noire</option>
                            <option value="Ostergotland">Ostergotland</option>
                            <option value="Vastmanland">Vastmanland</option>
                            <option value="Qena">Qena</option>
                            <option value="Gdansk">Gdansk</option>
                            <option value="Kygyzstan">Kygyzstan</option>
                            <option value="Gazankulu">Gazankulu</option>
                            <option value="Upper Austria">Upper Austria</option>
                            <option value="Kinkkale">Kinkkale</option>
                            <option value="Zaire">Zaire</option>
                            <option value="Gao">Gao</option>
                            <option value="Tirana">Tirana</option>
                            <option value="Palawan">Palawan</option>
                            <option value="Akita">Akita</option>
                            <option value="Imbabura">Imbabura</option>
                            <option value="Antique">Antique</option>
                            <option value="Artvin">Artvin</option>
                            <option value="Ardahan">Ardahan</option>
                            <option value="Ukraine">Ukraine</option>
                            <option value="Famagusta">Famagusta</option>
                            <option value="Kyrenia">Kyrenia</option>
                            <option value="Rondonia">Rondonia</option>
                            <option value="Grad Sofia">Grad Sofia</option>
                            <option value="Paphos">Paphos</option>
                            <option value="Surin">Surin</option>
                            <option value="West Sepik">West Sepik</option>
                            <option value="Miyagi">Miyagi</option>
                            <option value="Yamanashi">Yamanashi</option>
                            <option value="Iwate">Iwate</option>
                            <option value="Mie">Mie</option>
                            <option value="Sichuan">Sichuan</option>
                            <option value="Penang">Penang</option>
                            <option value="Kunduz">Kunduz</option>
                            <option value="Beijing">Beijing</option>
                            <option value="Constantine">Constantine</option>
                            <option value="Goias">Goias</option>
                            <option value="Mastaganem">Mastaganem</option>
                            <option value="Ruhengeri">Ruhengeri</option>
                            <option value="Agadez">Agadez</option>
                            <option value="Saxony">Saxony</option>
                            <option value="Niamey">Niamey</option>
                            <option value="Latvia">Latvia</option>
                            <option value="National Capital District">National Capital District</option>
                            <option value="Cayo">Cayo</option>
                            <option value="Katsina">Katsina</option>
                            <option value="Sagaing">Sagaing</option>
                            <option value="Incheon">Incheon</option>
                            <option value="Northwest">Northwest</option>
                            <option value="Tlemcen">Tlemcen</option>
                            <option value="Lower Juba">Lower Juba</option>
                            <option value="Estonia">Estonia</option>
                            <option value="Timbuktu">Timbuktu</option>
                            <option value="Northern Bahr el Ghazal">Northern Bahr el Ghazal</option>
                            <option value="Mopti">Mopti</option>
                            <option value="Beni Suef">Beni Suef</option>
                            <option value="Quena">Quena</option>
                            <option value="Peloponnese">Peloponnese</option>
                            <option value="Tillaberi">Tillaberi</option>
                            <option value="Souk Ahras">Souk Ahras</option>
                            <option value="Lithuania">Lithuania</option>
                            <option value="Havana">Havana</option>
                            <option value="Bouira">Bouira</option>
                            <option value="Valverde">Valverde</option>
                            <option value="Paktia">Paktia</option>
                            <option value="Osijek-Baranja">Osijek-Baranja</option>
                            <option value="Adamawa">Adamawa</option>
                            <option value="Pozega-Slavonia">Pozega-Slavonia</option>
                            <option value="Sisak-Moslavina">Sisak-Moslavina</option>
                            <option value="Rizal">Rizal</option>
                            <option value="Lika-Senj">Lika-Senj</option>
                            <option value="Marib">Marib</option>
                            <option value="Zagreb">Zagreb</option>
                            <option value="Split-Dalmatia">Split-Dalmatia</option>
                            <option value="Helmand">Helmand</option>
                            <option value="Bosnia-Herzegovina">Bosnia-Herzegovina</option>
                            <option value="San Cristobal">San Cristobal</option>
                            <option value="Dagestan">Dagestan</option>
                            <option value="Giza">Giza</option>
                            <option value="Aurora">Aurora</option>
                            <option value="Vychodocesky">Vychodocesky</option>
                            <option value="Brod-Posavina">Brod-Posavina</option>
                            <option value="Karlovac">Karlovac</option>
                            <option value="Zamfara">Zamfara</option>
                            <option value="Santo Domingo">Santo Domingo</option>
                            <option value="Byumba">Byumba</option>
                            <option value="Shida Kartli">Shida Kartli</option>
                            <option value="Tropoje">Tropoje</option>
                            <option value="Ordubad">Ordubad</option>
                            <option value="Tbilisi">Tbilisi</option>
                            <option value="Gilgit-Baltistan">Gilgit-Baltistan</option>
                            <option value="Mecklenburg-West Pomerania">Mecklenburg-West Pomerania</option>
                            <option value="Roraima">Roraima</option>
                            <option value="Alagoas">Alagoas</option>
                            <option value="Kano">Kano</option>
                            <option value="Ciudad Real">Ciudad Real</option>
                            <option value="Ayeyarwady">Ayeyarwady</option>
                            <option value="Mus">Mus</option>
                            <option value="Antisiranana">Antisiranana</option>
                            <option value="Thuringia">Thuringia</option>
                            <option value="Oriental">Oriental</option>
                            <option value="Vilnius">Vilnius</option>
                            <option value="Artibonite">Artibonite</option>
                            <option value="Rakhine">Rakhine</option>
                            <option value="Montserrado">Montserrado</option>
                            <option value="Santiago del Estero">Santiago del Estero</option>
                            <option value="Vukovar-Srijem">Vukovar-Srijem</option>
                            <option value="Madre de Dios">Madre de Dios</option>
                            <option value="Khojavend">Khojavend</option>
                            <option value="Bujumbura Mairie">Bujumbura Mairie</option>
                            <option value="Baku">Baku</option>
                            <option value="El Oued">El Oued</option>
                            <option value="Cibitoke">Cibitoke</option>
                            <option value="Bubanza">Bubanza</option>
                            <option value="Kharkiv">Kharkiv</option>
                            <option value="Skane">Skane</option>
                            <option value="Kiev">Kiev</option>
                            <option value="Tipaza">Tipaza</option>
                            <option value="Biskra">Biskra</option>
                            <option value="Transnistria">Transnistria</option>
                            <option value="Meghri">Meghri</option>
                            <option value="Kampong Cham">Kampong Cham</option>
                            <option value="Kyustendil">Kyustendil</option>
                            <option value="Hsinchu">Hsinchu</option>
                            <option value="Bari">Bari</option>
                            <option value="Centrale">Centrale</option>
                            <option value="Chechnya">Chechnya</option>
                            <option value="Plateaux">Plateaux</option>
                            <option value="Kampong Thom">Kampong Thom</option>
                            <option value="Nuevo Leon">Nuevo Leon</option>
                            <option value="Lower Austria">Lower Austria</option>
                            <option value="Tadjourah">Tadjourah</option>
                            <option value="Badakhshan">Badakhshan</option>
                            <option value="Lofa">Lofa</option>
                            <option value="Lagunes">Lagunes</option>
                            <option value="Antananarivo">Antananarivo</option>
                            <option value="Laghouat">Laghouat</option>
                            <option value="South Kazakhstan">South Kazakhstan</option>
                            <option value="Gelderland">Gelderland</option>
                            <option value="Batna">Batna</option>
                            <option value="Chon Buri">Chon Buri</option>
                            <option value="Xinjiang">Xinjiang</option>
                            <option value="Chachoengsao">Chachoengsao</option>
                            <option value="Nyanza">Nyanza</option>
                            <option value="Boumerdes">Boumerdes</option>
                            <option value="Saint Petersburg">Saint Petersburg</option>
                            <option value="Red Sea">Red Sea</option>
                            <option value="Sidi Bel Abbes">Sidi Bel Abbes</option>
                            <option value="Akwa Ibom">Akwa Ibom</option>
                            <option value="Agusan del Sur">Agusan del Sur</option>
                            <option value="Nord-Pas-de-Calais">Nord-Pas-de-Calais</option>
                            <option value="Samegrelo-Zemo Svaneti">Samegrelo-Zemo Svaneti</option>
                            <option value="Abkhazia">Abkhazia</option>
                            <option value="West Panama">West Panama</option>
                            <option value="Sarangani">Sarangani</option>
                            <option value="Edo">Edo</option>
                            <option value="Kigali">Kigali</option>
                            <option value="Saint Joseph">Saint Joseph</option>
                            <option value="Ticino">Ticino</option>
                            <option value="Shusha">Shusha</option>
                            <option value="Tartar">Tartar</option>
                            <option value="Kalbajar">Kalbajar</option>
                            <option value="Khojaly">Khojaly</option>
                            <option value="Saint John Parish">Saint John Parish</option>
                            <option value="Artik">Artik</option>
                            <option value="Zadar">Zadar</option>
                            <option value="Gisenyi">Gisenyi</option>
                            <option value="Taraba">Taraba</option>
                            <option value="Moscow">Moscow</option>
                            <option value="Iferouane">Iferouane</option>
                            <option value="Medea">Medea</option>
                            <option value="Shkoder">Shkoder</option>
                            <option value="Tochigi">Tochigi</option>
                            <option value="Khankendi">Khankendi</option>
                            <option value="Sud">Sud</option>
                            <option value="Grand Cape Mount">Grand Cape Mount</option>
                            <option value="Dnipropetrovsk">Dnipropetrovsk</option>
                            <option value="Edirne">Edirne</option>
                            <option value="Hadramawt">Hadramawt</option>
                            <option value="Ghor">Ghor</option>
                            <option value="San Juan-Laventille">San Juan-Laventille</option>
                            <option value="Sanaag">Sanaag</option>
                            <option value="Republika Srpska">Republika Srpska</option>
                            <option value="Bender">Bender</option>
                            <option value="Anenii Noi">Anenii Noi</option>
                            <option value="Johor">Johor</option>
                            <option value="Wardak">Wardak</option>
                            <option value="Marrakesh-Tensift-El Haouz">Marrakesh-Tensift-El Haouz</option>
                            <option value="Kaohsiung">Kaohsiung</option>
                            <option value="East">East</option>
                            <option value="New Taipei">New Taipei</option>
                            <option value="Federation of Bosnia and Herzegovina">Federation of Bosnia and Herzegovina</option>
                            <option value="Brcko">Brcko</option>
                            <option value="Gitarama">Gitarama</option>
                            <option value="Lavalleja">Lavalleja</option>
                            <option value="Dushanbe">Dushanbe</option>
                            <option value="Setif">Setif</option>
                            <option value="Limon">Limon</option>
                            <option value="Tashkent">Tashkent</option>
                            <option value="Jurmala">Jurmala</option>
                            <option value="Garkalne">Garkalne</option>
                            <option value="South Jeolla">South Jeolla</option>
                            <option value="Ondo">Ondo</option>
                            <option value="Taurage">Taurage</option>
                            <option value="Yunnan">Yunnan</option>
                            <option value="Bomi">Bomi</option>
                            <option value="Markazi">Markazi</option>
                            <option value="Kurdamir">Kurdamir</option>
                            <option value="Kutahya">Kutahya</option>
                            <option value="Western Highlands">Western Highlands</option>
                            <option value="Champasak">Champasak</option>
                            <option value="Belait">Belait</option>
                            <option value="Picardy">Picardy</option>
                            <option value="Mostaganem">Mostaganem</option>
                            <option value="Guelma">Guelma</option>
                            <option value="Luxor">Luxor</option>
                            <option value="Ain Defla">Ain Defla</option>
                            <option value="Bejaia">Bejaia</option>
                            <option value="Madang">Madang</option>
                            <option value="Gyumri">Gyumri</option>
                            <option value="Stung Treng">Stung Treng</option>
                            <option value="Takeo">Takeo</option>
                            <option value="Causeni">Causeni</option>
                            <option value="Es Semara">Es Semara</option>
                            <option value="Sainyabuli">Sainyabuli</option>
                            <option value="Pailin">Pailin</option>
                            <option value="Arbil">Arbil</option>
                            <option value="Harju">Harju</option>
                            <option value="Diyala">Diyala</option>
                            <option value="Banteay Meanchey">Banteay Meanchey</option>
                            <option value="Kep">Kep</option>
                            <option value="Siem Reap">Siem Reap</option>
                            <option value="Damietta">Damietta</option>
                            <option value="Samtskhe-Javakheti">Samtskhe-Javakheti</option>
                            <option value="Tiaret">Tiaret</option>
                            <option value="Jijel">Jijel</option>
                            <option value="Louga">Louga</option>
                            <option value="Djelfa">Djelfa</option>
                            <option value="Beheira">Beheira</option>
                            <option value="Logone Oriental">Logone Oriental</option>
                            <option value="New Valley">New Valley</option>
                            <option value="saraburi">saraburi</option>
                            <option value="Lopburi">Lopburi</option>
                            <option value="Shabwah">Shabwah</option>
                            <option value="Ho Chi Minh">Ho Chi Minh</option>
                            <option value="Cotopaxi">Cotopaxi</option>
                            <option value="Port Said">Port Said</option>
                            <option value="Almaty">Almaty</option>
                            <option value="Kara">Kara</option>
                            <option value="Meghalaya">Meghalaya</option>
                            <option value="Prachinburi">Prachinburi</option>
                            <option value="Adjara">Adjara</option>
                            <option value="Hiiraan">Hiiraan</option>
                            <option value="Gedo">Gedo</option>
                            <option value="Kratie">Kratie</option>
                            <option value="Kabardino-Balkaria">Kabardino-Balkaria</option>
                            <option value="Duarte">Duarte</option>
                            <option value="Yamagata">Yamagata</option>
                            <option value="North Ossetia-Alania">North Ossetia-Alania</option>
                            <option value="Qadisiyah">Qadisiyah</option>
                            <option value="Ngazidja">Ngazidja</option>
                            <option value="Saint Mary">Saint Mary</option>
                            <option value="Enga">Enga</option>
                            <option value="Kandal">Kandal</option>
                            <option value="East Kazakhstan">East Kazakhstan</option>
                            <option value="Stung Trang">Stung Trang</option>
                            <option value="Sinop">Sinop</option>
                            <option value="Maysan">Maysan</option>
                            <option value="Shimane">Shimane</option>
                            <option value="Irkutsk">Irkutsk</option>
                            <option value="Bong">Bong</option>
                            <option value="Hiroshima">Hiroshima</option>
                            <option value="Bay">Bay</option>
                            <option value="Mambere Kadei">Mambere Kadei</option>
                            <option value="Khatlon">Khatlon</option>
                            <option value="Burgenland">Burgenland</option>
                            <option value="Ratchaburi">Ratchaburi</option>
                            <option value="Districts of Republican Subordination">Districts of Republican Subordination</option>
                            <option value="Chlef">Chlef</option>
                            <option value="Yambol">Yambol</option>
                            <option value="Lower Shebelle">Lower Shebelle</option>
                            <option value="Prey Veng">Prey Veng</option>
                            <option value="Northern Territory">Northern Territory</option>
                            <option value="Alto Parana">Alto Parana</option>
                            <option value="Poznan">Poznan</option>
                            <option value="Littoral">Littoral</option>
                            <option value="Dushanabe">Dushanabe</option>
                            <option value="Kolda">Kolda</option>
                            <option value="Guangdong">Guangdong</option>
                            <option value="Beni">Beni</option>
                            <option value="Pavlodar">Pavlodar</option>
                            <option value="Dhi Qar">Dhi Qar</option>
                            <option value="Khost">Khost</option>
                            <option value="Tebessa">Tebessa</option>
                            <option value="Bratislava">Bratislava</option>
                            <option value="Yunlin">Yunlin</option>
                            <option value="Veraguas">Veraguas</option>
                            <option value="Karaganda">Karaganda</option>
                            <option value="Butare">Butare</option>
                            <option value="Sanaa">Sanaa</option>
                            <option value="Relizane">Relizane</option>
                            <option value="Extreme-North">Extreme-North</option>
                            <option value="Nzerekore">Nzerekore</option>
                            <option value="Lahij">Lahij</option>
                            <option value="Kvemo Kartli">Kvemo Kartli</option>
                            <option value="Middle Juba">Middle Juba</option>
                            <option value="Novosibirsk">Novosibirsk</option>
                            <option value="Kirikkale">Kirikkale</option>
                            <option value="Crimea">Crimea</option>
                            <option value="Fes-Boulemane">Fes-Boulemane</option>
                            <option value="Mascara">Mascara</option>
                            <option value="Qalyubia">Qalyubia</option>
                            <option value="Middle Shebelle">Middle Shebelle</option>
                            <option value="Tizi Ouzou">Tizi Ouzou</option>
                            <option value="Queretaro">Queretaro</option>
                            <option value="Bayelsa">Bayelsa</option>
                            <option value="Aswan">Aswan</option>
                            <option value="Al Bayda">Al Bayda</option>
                            <option value="Pursa">Pursa</option>
                            <option value="Skikda">Skikda</option>
                            <option value="Virovitica-Podravina">Virovitica-Podravina</option>
                            <option value="Baskortostan">Baskortostan</option>
                            <option value="Amur">Amur</option>
                            <option value="Kostroma">Kostroma</option>
                            <option value="Manzandaran">Manzandaran</option>
                            <option value="Amran">Amran</option>
                            <option value="Riga">Riga</option>
                            <option value="Jelenia Gora">Jelenia Gora</option>
                            <option value="Labe">Labe</option>
                            <option value="Ingushetia">Ingushetia</option>
                            <option value="Pazardzhik">Pazardzhik</option>
                            <option value="Kymenlaakso">Kymenlaakso</option>
                            <option value="Guanajuato">Guanajuato</option>
                            <option value="Pskov">Pskov</option>
                            <option value="Trnava">Trnava</option>
                            <option value="Maradi">Maradi</option>
                            <option value="Grand Bassa">Grand Bassa</option>
                            <option value="Dosso">Dosso</option>
                            <option value="Grevenmacher">Grevenmacher</option>
                            <option value="Sverdlovsk">Sverdlovsk</option>
                            <option value="Yerevan">Yerevan</option>
                            <option value="Primorsky Krai">Primorsky Krai</option>
                            <option value="Pursat">Pursat</option>
                            <option value="Diekirch">Diekirch</option>
                            <option value="Sonora">Sonora</option>
                            <option value="Kumanovo">Kumanovo</option>
                            <option value="Annaba">Annaba</option>
                            <option value="Baja California">Baja California</option>
                            <option value="Bas-Congo">Bas-Congo</option>
                            <option value="Phuket">Phuket</option>
                            <option value="Santo Domingo de los Tsachilas">Santo Domingo de los Tsachilas</option>
                            <option value="La Vega">La Vega</option>
                            <option value="Bamako">Bamako</option>
                            <option value="Sikkim">Sikkim</option>
                            <option value="Sarande">Sarande</option>
                            <option value="Ngozi">Ngozi</option>
                            <option value="Upper Normandy">Upper Normandy</option>
                            <option value="Buryatia">Buryatia</option>
                            <option value="Tunapuna-Piarco">Tunapuna-Piarco</option>
                            <option value="Cross River">Cross River</option>
                            <option value="Phongsaly">Phongsaly</option>
                            <option value="El Oro">El Oro</option>
                            <option value="Sevastopol">Sevastopol</option>
                            <option value="Distrito Federal">Distrito Federal</option>
                            <option value="Fujian">Fujian</option>
                            <option value="West Java">West Java</option>
                            <option value="Uvea">Uvea</option>
                            <option value="Qusar">Qusar</option>
                            <option value="Minsk">Minsk</option>
                            <option value="Mugla">Mugla</option>
                            <option value="Gambela">Gambela</option>
                            <option value="Stavropol">Stavropol</option>
                            <option value="Chita">Chita</option>
                            <option value="M\Sila">M\Sila</option>
                            <option value="Kampot">Kampot</option>
                            <option value="Voru">Voru</option>
                            <option value="El Tarf">El Tarf</option>
                            <option value="Montenegro">Montenegro</option>
                            <option value="Notranjsko-kraška">Notranjsko-kraška</option>
                            <option value="Ida-Viru">Ida-Viru</option>
                            <option value="Abuja">Abuja</option>
                            <option value="Keelung">Keelung</option>
                            <option value="Muyinga">Muyinga</option>
                            <option value="Noyemberyan">Noyemberyan</option>
                            <option value="Al Anbar">Al Anbar</option>
                            <option value="Auckland">Auckland</option>
                            <option value="Braganca">Braganca</option>
                            <option value="Oum El Bouaghi">Oum El Bouaghi</option>
                            <option value="Sari Pul">Sari Pul</option>
                            <option value="Lunda Sul">Lunda Sul</option>
                            <option value="Moyen-Cavally">Moyen-Cavally</option>
                            <option value="Vorarlberg">Vorarlberg</option>
                            <option value="Nippes">Nippes</option>
                            <option value="Taichung">Taichung</option>
                            <option value="Villa Clara">Villa Clara</option>
                            <option value="Goa">Goa</option>
                            <option value="Portland">Portland</option>
                            <option value="Kampong Chhnang">Kampong Chhnang</option>
                            <option value="Cyangugu">Cyangugu</option>
                            <option value="North Kivu">North Kivu</option>
                            <option value="Banjul">Banjul</option>
                            <option value="Mid-Western">Mid-Western</option>
                            <option value="Svay Rieng">Svay Rieng</option>
                            <option value="El Bayadh">El Bayadh</option>
                            <option value="Kirundo">Kirundo</option>
                            <option value="Kayanza">Kayanza</option>
                            <option value="Preah Vihear">Preah Vihear</option>
                            <option value="Komi Republic">Komi Republic</option>
                            <option value="Kumamoto">Kumamoto</option>
                            <option value="Gorno-Badakhshan">Gorno-Badakhshan</option>
                            <option value="Pyongyang">Pyongyang</option>
                            <option value="Maranhao">Maranhao</option>
                            <option value="Oromia">Oromia</option>
                            <option value="Maan">Maan</option>
                            <option value="Kibungu">Kibungu</option>
                            <option value="Chongqing">Chongqing</option>
                            <option value="Kampong Chnnang">Kampong Chnnang</option>
                            <option value="Bujumbura Rural">Bujumbura Rural</option>
                            <option value="Mato Grosso">Mato Grosso</option>
                            <option value="Bururi">Bururi</option>
                            <option value="Grand\Anse">Grand\Anse</option>
                            <option value="Tozeur">Tozeur</option>
                            <option value="Mila">Mila</option>
                            <option value="Nugaal">Nugaal</option>
                            <option value="Gikongoro">Gikongoro</option>
                            <option value="Diffa">Diffa</option>
                            <option value="Denkalia">Denkalia</option>
                            <option value="Saint Gallen">Saint Gallen</option>
                            <option value="Kanem">Kanem</option>
                            <option value="Carchi">Carchi</option>
                            <option value="Gitega">Gitega</option>
                            <option value="Khabarovsk Krai">Khabarovsk Krai</option>
                            <option value="Sud-Est">Sud-Est</option>
                            <option value="Matam">Matam</option>
                            <option value="Koh Kong">Koh Kong</option>
                            <option value="South Kivu">South Kivu</option>
                            <option value="Prachuap Khiri Khan">Prachuap Khiri Khan</option>
                            <option value="Muthanna">Muthanna</option>
                            <option value="Phra Nakhon Si Ayutthaya">Phra Nakhon Si Ayutthaya</option>
                            <option value="Plovdiv">Plovdiv</option>
                            <option value="El Taref">El Taref</option>
                            <option value="Vysocina">Vysocina</option>
                            <option value="Sughd">Sughd</option>
                            <option value="Odessa">Odessa</option>
                            <option value="Arauco">Arauco</option>
                            <option value="Mahajanga">Mahajanga</option>
                            <option value="Southern Nations">Southern Nations</option>
                            <option value="Szczecin">Szczecin</option>
                            <option value="South Carolina">South Carolina</option>
                            <option value="Hokkaido">Hokkaido</option>
                            <option value="Saint George Basseterre">Saint George Basseterre</option>
                            <option value="Princes Town">Princes Town</option>
                            <option value="Sakha">Sakha</option>
                            <option value="Muramvya">Muramvya</option>
                            <option value="Banska Bystrica">Banska Bystrica</option>
                            <option value="Tavush">Tavush</option>
                            <option value="Racha-Lechkhumi and Kvemo Svaneti">Racha-Lechkhumi and Kvemo Svaneti</option>
                            <option value="Morogoro">Morogoro</option>
                            <option value="Milne Bay">Milne Bay</option>
                            <option value="Makamba">Makamba</option>
                            <option value="Papeete">Papeete</option>
                            <option value="Wyoming">Wyoming</option>
                            <option value="Imereti">Imereti</option>
                            <option value="Skopje">Skopje</option>
                            <option value="Rio Negro">Rio Negro</option>
                            <option value="Zanzan">Zanzan</option>
                            <option value="Primorje-Gorski Kotar">Primorje-Gorski Kotar</option>
                            <option value="Tabuk">Tabuk</option>
                            <option value="Agneby">Agneby</option>
                            <option value="Fromager">Fromager</option>
                            <option value="Lam Dong">Lam Dong</option>
                            <option value="West Sumatra">West Sumatra</option>
                            <option value="Cankuzo">Cankuzo</option>
                            <option value="Styria">Styria</option>
                            <option value="Bordj Bour Arreridj">Bordj Bour Arreridj</option>
                            <option value="Macau">Macau</option>
                            <option value="Karbala">Karbala</option>
                            <option value="Monufia">Monufia</option>
                            <option value="Chisinau">Chisinau</option>
                            <option value="Khanty-Mansi">Khanty-Mansi</option>
                            <option value="Kukes">Kukes</option>
                            <option value="Hidalgo">Hidalgo</option>
                            <option value="Atacora">Atacora</option>
                            <option value="Champagne-Ardenne">Champagne-Ardenne</option>
                            <option value="Ibb">Ibb</option>
                            <option value="Hainan">Hainan</option>
                            <option value="Taizz">Taizz</option>
                            <option value="Bishkek">Bishkek</option>
                            <option value="Kassala">Kassala</option>
                            <option value="Copperbelt">Copperbelt</option>
                            <option value="Alberta">Alberta</option>
                            <option value="Muharraq">Muharraq</option>
                            <option value="Kampong Speu">Kampong Speu</option>
                            <option value="Zabaykalsky Krai">Zabaykalsky Krai</option>
                            <option value="Xinjiang Uyghur">Xinjiang Uyghur</option>
                            <option value="Bago">Bago</option>
                            <option value="Irbid">Irbid</option>
                            <option value="Rutana">Rutana</option>
                            <option value="Nord-Ouest">Nord-Ouest</option>
                            <option value="Klaipeda">Klaipeda</option>
                            <option value="Maranhaos">Maranhaos</option>
                            <option value="Suria">Suria</option>
                            <option value="Battmabang">Battmabang</option>
                            <option value="Derna">Derna</option>
                            <option value="Balkh">Balkh</option>
                            <option value="Baoruco">Baoruco</option>
                            <option value="Chelyabinsk">Chelyabinsk</option>
                            <option value="Vorenezh">Vorenezh</option>
                            <option value="Coahuila">Coahuila</option>
                            <option value="Kinmen">Kinmen</option>
                            <option value="Cuvette">Cuvette</option>
                            <option value="Volgograd">Volgograd</option>
                            <option value="Hanoi">Hanoi</option>
                            <option value="Astrakhan">Astrakhan</option>
                            <option value="Quintana Roo">Quintana Roo</option>
                            <option value="Shaanxi">Shaanxi</option>
                            <option value="Dakhlet Nouadhibou">Dakhlet Nouadhibou</option>
                            <option value="Darien">Darien</option>
                            <option value="Shandong">Shandong</option>
                            <option value="Durango">Durango</option>
                            <option value="Yogyakarta">Yogyakarta</option>
                            <option value="Lakes">Lakes</option>
                            <option value="Suez">Suez</option>
                            <option value="Dubrovnik-Neretva">Dubrovnik-Neretva</option>
                            <option value="Ruyigi">Ruyigi</option>
                            <option value="Jiangxi">Jiangxi</option>
                            <option value="Sibenik-Knin">Sibenik-Knin</option>
                            <option value="Maribor">Maribor</option>
                            <option value="Saint James">Saint James</option>
                            <option value="Skierniewice">Skierniewice</option>
                            <option value="Komarom-Esztergom">Komarom-Esztergom</option>
                            <option value="Southern Highlands">Southern Highlands</option>
                            <option value="Haskovo">Haskovo</option>
                            <option value="Rio Grande do Norte">Rio Grande do Norte</option>
                            <option value="Antsiranana">Antsiranana</option>
                            <option value="Velenje">Velenje</option>
                            <option value="Phichit">Phichit</option>
                            <option value="Samut Sakhon">Samut Sakhon</option>
                            <option value="Nakhon Pathom">Nakhon Pathom</option>
                            <option value="Nakhon Sawan">Nakhon Sawan</option>
                            <option value="St. Petersburg">St. Petersburg</option>
                            <option value="Tyumen">Tyumen</option>
                            <option value="Moravian-Silesian">Moravian-Silesian</option>
                            <option value="Wadi al Shatii">Wadi al Shatii</option>
                            <option value="Ashanti">Ashanti</option>
                            <option value="Stara Zagora">Stara Zagora</option>
                            <option value="Saida">Saida</option>
                            <option value="Gunma">Gunma</option>
                            <option value="Tibet">Tibet</option>
                            <option value="Northern Red Sea">Northern Red Sea</option>
                            <option value="Ceara">Ceara</option>
                            <option value="Tatarstan">Tatarstan</option>
                            <option value="Harer">Harer</option>
                            <option value="Oddar Meancheay">Oddar Meancheay</option>
                            <option value="Kochi">Kochi</option>
                            <option value="Arusha">Arusha</option>
                            <option value="Vojvodina">Vojvodina</option>
                            <option value="Gharbia">Gharbia</option>
                            <option value="Nzwani">Nzwani</option>
                            <option value="Presov">Presov</option>
                            <option value="Elbasan">Elbasan</option>
                            <option value="Sangha">Sangha</option>
                            <option value="South Darfur">South Darfur</option>
                            <option value="Cojedes">Cojedes</option>
                            <option value="Blida Province">Blida Province</option>
                            <option value="Medea Province">Medea Province</option>
                            <option value="Central Java">Central Java</option>
                            <option value="Guajira">Guajira</option>
                            <option value="Matanzas">Matanzas</option>
                            <option value="Copan">Copan</option>
                            <option value="Bechar">Bechar</option>
                            <option value="Krasnodar">Krasnodar</option>
                            <option value="Algiers Province">Algiers Province</option>
                            <option value="Dhamar">Dhamar</option>
                            <option value="Hajjah">Hajjah</option>
                            <option value="Sihanoukville">Sihanoukville</option>
                            <option value="Legnica">Legnica</option>
                            <option value="Oyo">Oyo</option>
                            <option value="Anambra">Anambra</option>
                            <option value="Zacatecas">Zacatecas</option>
                            <option value="Durres">Durres</option>
                            <option value="Maria Trinidad Sanchez">Maria Trinidad Sanchez</option>
                            <option value="Saint John">Saint John</option>
                            <option value="Puke">Puke</option>
                            <option value="Lezhe">Lezhe</option>
                            <option value="Tissimsilt">Tissimsilt</option>
                            <option value="Kibuye">Kibuye</option>
                            <option value="Fier">Fier</option>
                            <option value="Gjirokaster">Gjirokaster</option>
                            <option value="Lushnje">Lushnje</option>
                            <option value="Novgorod">Novgorod</option>
                            <option value="Peqin">Peqin</option>
                            <option value="Moyen-Chari">Moyen-Chari</option>
                            <option value="Diber">Diber</option>
                            <option value="Ain Temouchent">Ain Temouchent</option>
                            <option value="Ain Tecmouchent">Ain Tecmouchent</option>
                            <option value="Aguascalientes">Aguascalientes</option>
                            <option value="Kirsehir">Kirsehir</option>
                            <option value="Szabolcs-Szatmar-Bereg">Szabolcs-Szatmar-Bereg</option>
                            <option value="Rivers">Rivers</option>
                            <option value="Vlore">Vlore</option>
                            <option value="Abyan">Abyan</option>
                            <option value="Galguduud">Galguduud</option>
                            <option value="Kucove">Kucove</option>
                            <option value="Osun">Osun</option>
                            <option value="Mwali">Mwali</option>
                            <option value="Leningrad">Leningrad</option>
                            <option value="Tissemsilt">Tissemsilt</option>
                            <option value="Haut-Ogooue">Haut-Ogooue</option>
                            <option value="Obock">Obock</option>
                            <option value="Ekiti">Ekiti</option>
                            <option value="Kosovo and Metohija">Kosovo and Metohija</option>
                            <option value="Kalmykia">Kalmykia</option>
                            <option value="South Sulawesi">South Sulawesi</option>
                            <option value="Delta">Delta</option>
                            <option value="Tepelene">Tepelene</option>
                            <option value="Ljubljana">Ljubljana</option>
                            <option value="Medimurje">Medimurje</option>
                            <option value="Central Bosnia">Central Bosnia</option>
                            <option value="Beijaia">Beijaia</option>
                            <option value="Bakool">Bakool</option>
                            <option value="Manitoba">Manitoba</option>
                            <option value="Puntarenas">Puntarenas</option>
                            <option value="Karuzi">Karuzi</option>
                            <option value="North Kordofan">North Kordofan</option>
                            <option value="Los Santos">Los Santos</option>
                            <option value="Blue Nile">Blue Nile</option>
                            <option value="Henan">Henan</option>
                            <option value="Gostivar">Gostivar</option>
                            <option value="Saint Andrew">Saint Andrew</option>
                            <option value="Mallakaster">Mallakaster</option>
                            <option value="Tetovo">Tetovo</option>
                            <option value="Moscow (Federal City)">Moscow (Federal City)</option>
                            <option value="Kumanovo (Municipality)">Kumanovo (Municipality)</option>
                            <option value="Prilep (Municipality)">Prilep (Municipality)</option>
                            <option value="Maramvya">Maramvya</option>
                            <option value="Relizane (Province)">Relizane (Province)</option>
                            <option value="Saida (Province)">Saida (Province)</option>
                            <option value="Bouira (Province)">Bouira (Province)</option>
                            <option value="Boussaken (Province)">Boussaken (Province)</option>
                            <option value="Muaskar (Province)">Muaskar (Province)</option>
                            <option value="Algiers (Province)">Algiers (Province)</option>
                            <option value="Constantine (Province)">Constantine (Province)</option>
                            <option value="Yekaterinburg">Yekaterinburg</option>
                            <option value="Dagestan (Republic)">Dagestan (Republic)</option>
                            <option value="Sofia City (Province)">Sofia City (Province)</option>
                            <option value="Narathiwat (Province)">Narathiwat (Province)</option>
                            <option value="El Biar (District)">El Biar (District)</option>
                            <option value="Setif (Province)">Setif (Province)</option>
                            <option value="Blida (Province)">Blida (Province)</option>
                            <option value="Djelfa (Province)">Djelfa (Province)</option>
                            <option value="Laghouat (Province)">Laghouat (Province)</option>
                            <option value="Kosice (Region)">Kosice (Region)</option>
                            <option value="Tlemcen (Province)">Tlemcen (Province)</option>
                            <option value="Medea (Province)">Medea (Province)</option>
                            <option value="Tbilisi (Capital City)">Tbilisi (Capital City)</option>
                            <option value="Lautem (District)">Lautem (District)</option>
                            <option value="Hubei (Province)">Hubei (Province)</option>
                            <option value="Yala (Province)">Yala (Province)</option>
                            <option value="Boumerdes (Province)">Boumerdes (Province)</option>
                            <option value="Samegrelo-Zemo Svaneti (Region)">Samegrelo-Zemo Svaneti (Region)</option>
                            <option value="Kratie (Province)">Kratie (Province)</option>
                            <option value="Tak (Province)">Tak (Province)</option>
                            <option value="Budapest (Capital City)">Budapest (Capital City)</option>
                            <option value="Al Qahirah (Governorate)">Al Qahirah (Governorate)</option>
                            <option value="Karotegin (Province)">Karotegin (Province)</option>
                            <option value="Tizi Ouzau">Tizi Ouzau</option>
                            <option value="Oran (Province)">Oran (Province)</option>
                            <option value="Abkhazia (Autonomous Republic)">Abkhazia (Autonomous Republic)</option>
                            <option value="Riga (City)">Riga (City)</option>
                            <option value="M\sila (Province)">M\sila (Province)</option>
                            <option value="Bejaia (Province)">Bejaia (Province)</option>
                            <option value="Chechnya (Republic)">Chechnya (Republic)</option>
                            <option value="Kosovo (Province)">Kosovo (Province)</option>
                            <option value="Tipasa">Tipasa</option>
                            <option value="Wicklow">Wicklow</option>
                            <option value="Ain Defla (Province)">Ain Defla (Province)</option>
                            <option value="Crna Gora (Republic)">Crna Gora (Republic)</option>
                            <option value="Macau (Special Administrative Region)">Macau (Special Administrative Region)</option>
                            <option value="Bratislava (Region)">Bratislava (Region)</option>
                            <option value="Bangkok (District)">Bangkok (District)</option>
                            <option value="Guerrero (State)">Guerrero (State)</option>
                            <option value="Phom Penh (Municipality)">Phom Penh (Municipality)</option>
                            <option value="Al Minya (Governorate)">Al Minya (Governorate)</option>
                            <option value="Alberta (Province)">Alberta (Province)</option>
                            <option value="Siem Reap (Province)">Siem Reap (Province)</option>
                            <option value="Al Jawf">Al Jawf</option>
                            <option value="Skopje (Municipality)">Skopje (Municipality)</option>
                            <option value="Oaxaca (State)">Oaxaca (State)</option>
                            <option value="Yerevan (Special Administrative Region)">Yerevan (Special Administrative Region)</option>
                            <option value="Preah Vihear (Province)">Preah Vihear (Province)</option>
                            <option value="Al Hudaydah">Al Hudaydah</option>
                            <option value="Bosnia and Herzegovina (Federation)">Bosnia and Herzegovina (Federation)</option>
                            <option value="Irkutsk (Oblast)">Irkutsk (Oblast)</option>
                            <option value="Podlaskie (Region)">Podlaskie (Region)</option>
                            <option value="North Ossetia-Alania (Republic)">North Ossetia-Alania (Republic)</option>
                            <option value="Librazhd">Librazhd</option>
                            <option value="Pomeranian (Voivodeship)">Pomeranian (Voivodeship)</option>
                            <option value="Kulyabskaya (Oblast)">Kulyabskaya (Oblast)</option>
                            <option value="El-Oued (Province)">El-Oued (Province)</option>
                            <option value="Tipasa (Province)">Tipasa (Province)</option>
                            <option value="Arkhangelsk">Arkhangelsk</option>
                            <option value="Tiaret (Province)">Tiaret (Province)</option>
                            <option value="Chlef (Province)">Chlef (Province)</option>
                            <option value="Sidi Bel Abbes (Province)">Sidi Bel Abbes (Province)</option>
                            <option value="Tizi Ouzou (Province)">Tizi Ouzou (Province)</option>
                            <option value="Sakhalin (Oblast)">Sakhalin (Oblast)</option>
                            <option value="Ouest (Department)">Ouest (Department)</option>
                            <option value="Tipaza (Province)">Tipaza (Province)</option>
                            <option value="Ar Raqqah">Ar Raqqah</option>
                            <option value="Maniema">Maniema</option>
                            <option value="Dushanbe (Capital City)">Dushanbe (Capital City)</option>
                            <option value="Ain Temouchent (Province)">Ain Temouchent (Province)</option>
                            <option value="Auvergne">Auvergne</option>
                            <option value="New South Wales (State)">New South Wales (State)</option>
                            <option value="Khatlon (Province)">Khatlon (Province)</option>
                            <option value="Manufahi (District)">Manufahi (District)</option>
                            <option value="British Colombia">British Colombia</option>
                            <option value="St. Petersburg (Federal City)">St. Petersburg (Federal City)</option>
                            <option value="East Java (Province)">East Java (Province)</option>
                            <option value="Tipaza Province">Tipaza Province</option>
                            <option value="Ain Defla Province">Ain Defla Province</option>
                            <option value="Tebessa (Province)">Tebessa (Province)</option>
                            <option value="Tashkent (Capital City)">Tashkent (Capital City)</option>
                            <option value="Luganda">Luganda</option>
                            <option value="Skikda (Province)">Skikda (Province)</option>
                            <option value="Phnom Penh (Municipality)">Phnom Penh (Municipality)</option>
                            <option value="Cankiri">Cankiri</option>
                            <option value="Kampot (Province)">Kampot (Province)</option>
                            <option value="East Darfur">East Darfur</option>
                            <option value="Kabylie (Region)">Kabylie (Region)</option>
                            <option value="Jijel (Province)">Jijel (Province)</option>
                            <option value="Panama (Province)">Panama (Province)</option>
                            <option value="Karachay-Cherkessia (Republic)">Karachay-Cherkessia (Republic)</option>
                            <option value="Central Java (Province)">Central Java (Province)</option>
                            <option value="Niamey (Capital District)">Niamey (Capital District)</option>
                            <option value="Minsk (Capital City)">Minsk (Capital City)</option>
                            <option value="Kandal (Province)">Kandal (Province)</option>
                            <option value="Jakarta (Special Capital Region)">Jakarta (Special Capital Region)</option>
                            <option value="Ningxia Hui">Ningxia Hui</option>
                            <option value="Sverdlovsk (Oblast)">Sverdlovsk (Oblast)</option>
                            <option value="Irian Jaya (Province)">Irian Jaya (Province)</option>
                            <option value="Wasit">Wasit</option>
                            <option value="Dili (District)">Dili (District)</option>
                            <option value="Aceh (Special Territory)">Aceh (Special Territory)</option>
                            <option value="Belgrade (District)">Belgrade (District)</option>
                            <option value="Aceh (Province)">Aceh (Province)</option>
                            <option value="South Sulawesi (Province)">South Sulawesi (Province)</option>
                            <option value="Pool">Pool</option>
                            <option value="Harju (County)">Harju (County)</option>
                            <option value="Guandacanal">Guandacanal</option>
                            <option value="Boufarik (Province)">Boufarik (Province)</option>
                            <option value="Liquica (District)">Liquica (District)</option>
                            <option value="Pecki">Pecki</option>
                            <option value="Greater Skopje">Greater Skopje</option>
                            <option value="M\chedallah (District)">M\chedallah (District)</option>
                            <option value="Ingushetia (Republic)">Ingushetia (Republic)</option>
                            <option value="Kayin (State)">Kayin (State)</option>
                            <option value="Sichuan (Province)">Sichuan (Province)</option>
                            <option value="Kiev City (Municipality)">Kiev City (Municipality)</option>
                            <option value="North Aceh (Regency)">North Aceh (Regency)</option>
                            <option value="Cova-lima (District)">Cova-lima (District)</option>
                            <option value="Annaba (Province)">Annaba (Province)</option>
                            <option value="Bouenza">Bouenza</option>
                            <option value="Pidie (Regency)">Pidie (Regency)</option>
                            <option value="Batna (Province)">Batna (Province)</option>
                            <option value="Kukës (County)">Kukës (County)</option>
                            <option value="Bechar (Region)">Bechar (Region)</option>
                            <option value="Central (Division)">Central (Division)</option>
                            <option value="Gnjilane">Gnjilane</option>
                            <option value="Yangon (Division)">Yangon (Division)</option>
                            <option value="Batken (Oblast)">Batken (Oblast)</option>
                            <option value="Bobonaro (District)">Bobonaro (District)</option>
                            <option value="Ermera (District)">Ermera (District)</option>
                            <option value="East Nusa Tenggara (Province)">East Nusa Tenggara (Province)</option>
                            <option value="Shamal Sina\ (Governorate)">Shamal Sina\ (Governorate)</option>
                            <option value="Mon (State)">Mon (State)</option>
                            <option value="Sidi Bel Abbes (Region)">Sidi Bel Abbes (Region)</option>
                            <option value="Rostov (Oblast)">Rostov (Oblast)</option>
                            <option value="River Nile">River Nile</option>
                            <option value="Batken">Batken</option>
                            <option value="Maluka (Province)">Maluka (Province)</option>
                            <option value="Pristina">Pristina</option>
                            <option value="Atlantico Norte (Department)">Atlantico Norte (Department)</option>
                            <option value="Dnipropetrovsk (Oblast)">Dnipropetrovsk (Oblast)</option>
                            <option value="Fergana (Province)">Fergana (Province)</option>
                            <option value="Maradi (Region)">Maradi (Region)</option>
                            <option value="Tanintharyi (Division)">Tanintharyi (Division)</option>
                            <option value="Kolubara (District)">Kolubara (District)</option>
                            <option value="Northwestern">Northwestern</option>
                            <option value="Bosanska Krajina (Region)">Bosanska Krajina (Region)</option>
                            <option value="Ad Dali">Ad Dali</option>
                            <option value="Central Serbia (Region)">Central Serbia (Region)</option>
                            <option value="Fier (County)">Fier (County)</option>
                            <option value="L\viv (Oblast)">L\viv (Oblast)</option>
                            <option value="Tokyo (Prefecture)">Tokyo (Prefecture)</option>
                            <option value="Sofia City">Sofia City</option>
                            <option value="Draa El Mizan">Draa El Mizan</option>
                            <option value="El Khardja">El Khardja</option>
                            <option value="Cetinje (Municipality)">Cetinje (Municipality)</option>
                            <option value="Sukhumi (Region)">Sukhumi (Region)</option>
                            <option value="Mostaganem (Province)">Mostaganem (Province)</option>
                            <option value="Chechnya (Occupied Territory)">Chechnya (Occupied Territory)</option>
                            <option value="Orahovac (Municipality)">Orahovac (Municipality)</option>
                            <option value="Compostela Valley">Compostela Valley</option>
                            <option value="East Aceh">East Aceh</option>
                            <option value="Aracinovo (Municipality)">Aracinovo (Municipality)</option>
                            <option value="Naka District, Ibaraki">Naka District, Ibaraki</option>
                            <option value="Lower Normandy">Lower Normandy</option>
                            <option value="Astrakhan (Oblast)">Astrakhan (Oblast)</option>
                            <option value="Graubunden">Graubunden</option>
                            <option value="Oslomej (Municipality)">Oslomej (Municipality)</option>
                            <option value="Ratchaburi (Province)">Ratchaburi (Province)</option>
                            <option value="Smolensk (Oblast)">Smolensk (Oblast)</option>
                            <option value="Belgrade">Belgrade</option>
                            <option value="Isparta">Isparta</option>
                            <option value="Kumanovo municipality">Kumanovo municipality</option>
                            <option value="Kosovska Mitrovica">Kosovska Mitrovica</option>
                            <option value="Khabarovsk (Krai)">Khabarovsk (Krai)</option>
                            <option value="Beijing (Municipality)">Beijing (Municipality)</option>
                            <option value="Maluku (Province)">Maluku (Province)</option>
                            <option value="Tissemsilt (Province)">Tissemsilt (Province)</option>
                            <option value="Pcinja">Pcinja</option>
                            <option value="Kamchatka (Oblast)">Kamchatka (Oblast)</option>
                            <option value="Vedensky">Vedensky</option>
                            <option value="Sabah (State)">Sabah (State)</option>
                            <option value="Vientiane (Prefecture)">Vientiane (Prefecture)</option>
                            <option value="Vojvodina (Province)">Vojvodina (Province)</option>
                            <option value="Grand\Anse (Department)">Grand\Anse (Department)</option>
                            <option value="Tunis (Governorate)">Tunis (Governorate)</option>
                            <option value="Quebec (Province)">Quebec (Province)</option>
                            <option value="Bishkek (Capital City)">Bishkek (Capital City)</option>
                            <option value="Vukovar-Syrmia">Vukovar-Syrmia</option>
                            <option value="North Sumatra (Province)">North Sumatra (Province)</option>
                            <option value="Volgograd (Oblast)">Volgograd (Oblast)</option>
                            <option value="Podgorica">Podgorica</option>
                            <option value="Tokyo (Metropolis)">Tokyo (Metropolis)</option>
                            <option value="Central Serbia">Central Serbia</option>
                            <option value="Skikida (Province)">Skikida (Province)</option>
                            <option value="Montenegro (Republic)">Montenegro (Republic)</option>
                            <option value="Berber (Province)">Berber (Province)</option>
                            <option value="East Timor (Region)">East Timor (Region)</option>
                            <option value="Obilic (Municipality)">Obilic (Municipality)</option>
                            <option value="Topaz (Province)">Topaz (Province)</option>
                            <option value="Federal Territory">Federal Territory</option>
                            <option value="Biskra (Province)">Biskra (Province)</option>
                            <option value="Tissmesilt">Tissmesilt</option>
                            <option value="British Colombia (Province)">British Colombia (Province)</option>
                            <option value="Pec">Pec</option>
                            <option value="Mexican Federal District">Mexican Federal District</option>
                            <option value="Smolensk Oblast">Smolensk Oblast</option>
                            <option value="Cakaudrove">Cakaudrove</option>
                            <option value="Macuata">Macuata</option>
                            <option value="Hong Kong (Special Administrative Region)">Hong Kong (Special Administrative Region)
                            </option>
                            <option value="Karachay-Cherkessia">Karachay-Cherkessia</option>
                            <option value="Badghis">Badghis</option>
                            <option value="Gjilane">Gjilane</option>
                            <option value="Asir">Asir</option>
                            <option value="Prizren">Prizren</option>
                            <option value="East Nusa Tenggara">East Nusa Tenggara</option>
                            <option value="Lika">Lika</option>
                            <option value="Kindia">Kindia</option>
                            <option value="Pristina district">Pristina district</option>
                            <option value="Stavropol (Krai)">Stavropol (Krai)</option>
                            <option value="Urus-Martanovsky">Urus-Martanovsky</option>
                            <option value="Guerrouche (Province)">Guerrouche (Province)</option>
                            <option value="Cental Division">Cental Division</option>
                            <option value="Maluku">Maluku</option>
                            <option value="Dakovica">Dakovica</option>
                            <option value="Mitidja Plains (Province)">Mitidja Plains (Province)</option>
                            <option value="Almaty (Municipal District)">Almaty (Municipal District)</option>
                            <option value="Central Sulawesi (Province)">Central Sulawesi (Province)</option>
                            <option value="Inner Mongolia">Inner Mongolia</option>
                            <option value="Rukwa">Rukwa</option>
                            <option value="Kaliningrad Oblast">Kaliningrad Oblast</option>
                            <option value="Stavropol Krai">Stavropol Krai</option>
                            <option value="Orellana">Orellana</option>
                            <option value="Battambang (Province)">Battambang (Province)</option>
                            <option value="Jablanica (District)">Jablanica (District)</option>
                            <option value="Chisinau (Municipality)">Chisinau (Municipality)</option>
                            <option value="Central Division">Central Division</option>
                            <option value="Luhansk Oblast">Luhansk Oblast</option>
                            <option value="Ljubljana (Municipality)">Ljubljana (Municipality)</option>
                            <option value="Central Maluku">Central Maluku</option>
                            <option value="Hubei">Hubei</option>
                            <option value="New Territories">New Territories</option>
                            <option value="Bouïra Province">Bouïra Province</option>
                            <option value="Faranah">Faranah</option>
                            <option value="Takhar">Takhar</option>
                            <option value="Yogyakarta Special Region">Yogyakarta Special Region</option>
                            <option value="Riau (Province)">Riau (Province)</option>
                            <option value="West Java (Province)">West Java (Province)</option>
                            <option value="Pemba South">Pemba South</option>
                            <option value="Castries Quarter">Castries Quarter</option>
                            <option value="Vranje (Municipality)">Vranje (Municipality)</option>
                            <option value="Bamyan">Bamyan</option>
                            <option value="Western Bahr el Ghazal">Western Bahr el Ghazal</option>
                            <option value="North Seram (Sub district)">North Seram (Sub district)</option>
                            <option value="Dahra (Region)">Dahra (Region)</option>
                            <option value="Jakarta (Capital City District)">Jakarta (Capital City District)</option>
                            <option value="Tearce (Municipality)">Tearce (Municipality)</option>
                            <option value="Kanto">Kanto</option>
                            <option value="Bujanovac (Municipality)">Bujanovac (Municipality)</option>
                            <option value="Pcinja (District)">Pcinja (District)</option>
                            <option value="Yugoslavia">Yugoslavia</option>
                            <option value="Kedah (State)">Kedah (State)</option>
                            <option value="Yunnan (Province)">Yunnan (Province)</option>
                            <option value="Yeongnam">Yeongnam</option>
                            <option value="Dak Lak (Province)">Dak Lak (Province)</option>
                            <option value="Kigoma">Kigoma</option>
                            <option value="Akmola">Akmola</option>
                            <option value="Bissau">Bissau</option>
                            <option value="Kofarnihon (District)">Kofarnihon (District)</option>
                            <option value="Parita-Merivalja">Parita-Merivalja</option>
                            <option value="Vedeno">Vedeno</option>
                            <option value="Zagreb (City)">Zagreb (City)</option>
                            <option value="Banten (Province)">Banten (Province)</option>
                            <option value="Central Kalimantan">Central Kalimantan</option>
                            <option value="Tula (Oblast)">Tula (Oblast)</option>
                            <option value="Tetovo (Municipality)">Tetovo (Municipality)</option>
                            <option value="El Bordj (District)">El Bordj (District)</option>
                            <option value="Presevo (Municipality)">Presevo (Municipality)</option>
                            <option value="Taoyuan (County)">Taoyuan (County)</option>
                            <option value="Songkhla (Province)">Songkhla (Province)</option>
                            <option value="Muang">Muang</option>
                            <option value="Shali">Shali</option>
                            <option value="Siah">Siah</option>
                            <option value="Gisagara">Gisagara</option>
                            <option value="Skopje (Region)">Skopje (Region)</option>
                            <option value="Lipkovo (Municipality)">Lipkovo (Municipality)</option>
                            <option value="Guntur">Guntur</option>
                            <option value="North-Kivu">North-Kivu</option>
                            <option value="Lampung (Province)">Lampung (Province)</option>
                            <option value="Opstina Strumica (Region)">Opstina Strumica (Region)</option>
                            <option value="Zadar (County)">Zadar (County)</option>
                            <option value="Cunene">Cunene</option>
                            <option value="Bechar (Province)">Bechar (Province)</option>
                            <option value="Tomsk Oblast">Tomsk Oblast</option>
                            <option value="Seventh District">Seventh District</option>
                            <option value="Shirak">Shirak</option>
                            <option value="Monivong">Monivong</option>
                            <option value="Heredia">Heredia</option>
                            <option value="Jegunovce">Jegunovce</option>
                            <option value="Tearce">Tearce</option>
                            <option value="Tabaza (Province)">Tabaza (Province)</option>
                            <option value="Jakarta (Province)">Jakarta (Province)</option>
                            <option value="Batken (Province)">Batken (Province)</option>
                            <option value="Tissemselt (Province)">Tissemselt (Province)</option>
                            <option value="Mexico City (Federal District)">Mexico City (Federal District)</option>
                            <option value="Gostivar (Municipality)">Gostivar (Municipality)</option>
                            <option value="Polog">Polog</option>
                            <option value="Jegunovce (Municipality)">Jegunovce (Municipality)</option>
                            <option value="Northeastern">Northeastern</option>
                            <option value="Vargas">Vargas</option>
                            <option value="Liberec (Region)">Liberec (Region)</option>
                            <option value="Greater Skopje (Statistical Region)">Greater Skopje (Statistical Region)</option>
                            <option value="Kvemo Kartli (Region)">Kvemo Kartli (Region)</option>
                            <option value="Branicevo (District)">Branicevo (District)</option>
                            <option value="Urosevac">Urosevac</option>
                            <option value="Unity">Unity</option>
                            <option value="Zaqatala">Zaqatala</option>
                            <option value="Astrakhan (Federal Subject)">Astrakhan (Federal Subject)</option>
                            <option value="Masovian (Province)">Masovian (Province)</option>
                            <option value="Mascara (Province)">Mascara (Province)</option>
                            <option value="Greater Skopje (Special Division)">Greater Skopje (Special Division)</option>
                            <option value="Moluku (Province)">Moluku (Province)</option>
                            <option value="Skopje (District)">Skopje (District)</option>
                            <option value="Opoja (Municipality)">Opoja (Municipality)</option>
                            <option value="Federal (District)">Federal (District)</option>
                            <option value="Mwaro">Mwaro</option>
                            <option value="Yunlin (County)">Yunlin (County)</option>
                            <option value="Queensland (State)">Queensland (State)</option>
                            <option value="Ba (Province)">Ba (Province)</option>
                            <option value="Lupon">Lupon</option>
                            <option value="Zug">Zug</option>
                            <option value="Zaqatala (District)">Zaqatala (District)</option>
                            <option value="Chiba (Prefecture)">Chiba (Prefecture)</option>
                            <option value="Victoria (State)">Victoria (State)</option>
                            <option value="North Aceh (Province)">North Aceh (Province)</option>
                            <option value="Tearce municipality">Tearce municipality</option>
                            <option value="Kasserine">Kasserine</option>
                            <option value="Maluku province">Maluku province</option>
                            <option value="Ar Rayyan">Ar Rayyan</option>
                            <option value="Polog (Region)">Polog (Region)</option>
                            <option value="Tetovo Municipality">Tetovo Municipality</option>
                            <option value="Central Sulawesi">Central Sulawesi</option>
                            <option value="Gbarpolu">Gbarpolu</option>
                            <option value="Greater Wellington (Region)">Greater Wellington (Region)</option>
                            <option value="Chlef  (Province)">Chlef (Province)</option>
                            <option value="Kumanovo (Region)">Kumanovo (Region)</option>
                            <option value="Guangdong (Province)">Guangdong (Province)</option>
                            <option value="Shaanxi (Province)">Shaanxi (Province)</option>
                            <option value="Enugu">Enugu</option>
                            <option value="Shandong (Province)">Shandong (Province)</option>
                            <option value="Pattani (Province)">Pattani (Province)</option>
                            <option value="Kayar">Kayar</option>
                            <option value="Sulawesi (Province)">Sulawesi (Province)</option>
                            <option value="Lipkovo (Region)">Lipkovo (Region)</option>
                            <option value="Aceh Besar (Province)">Aceh Besar (Province)</option>
                            <option value="Rouiba (Province)">Rouiba (Province)</option>
                            <option value="Odessa (Oblast)">Odessa (Oblast)</option>
                            <option value="Bir Mourad Rais (Province)">Bir Mourad Rais (Province)</option>
                            <option value="Kosovo (State)">Kosovo (State)</option>
                            <option value="Haoudh (Region)">Haoudh (Region)</option>
                            <option value="Blidu (Province)">Blidu (Province)</option>
                            <option value="Kanagawa (Prefecture)">Kanagawa (Prefecture)</option>
                            <option value="Abkhazia (Region)">Abkhazia (Region)</option>
                            <option value="Kiev City Municipality">Kiev City Municipality</option>
                            <option value="Margibi">Margibi</option>
                            <option value="Medenine Governorate">Medenine Governorate</option>
                            <option value="Kayin State">Kayin State</option>
                            <option value="Borkou">Borkou</option>
                            <option value="Tiaret (Region)">Tiaret (Region)</option>
                            <option value="West Darfur">West Darfur</option>
                            <option value="North Ossetia">North Ossetia</option>
                            <option value="Chin (State)">Chin (State)</option>
                            <option value="Baku (City)">Baku (City)</option>
                            <option value="Karen (State)">Karen (State)</option>
                            <option value="Southeast (Department)">Southeast (Department)</option>
                            <option value="Shan (State)">Shan (State)</option>
                            <option value="Zakarpattia (Oblast)">Zakarpattia (Oblast)</option>
                            <option value="Fujian (Province)">Fujian (Province)</option>
                            <option value="Aomar (Province)">Aomar (Province)</option>
                            <option value="Fianarantsoa">Fianarantsoa</option>
                            <option value="Aceh province">Aceh province</option>
                            <option value="Tbilisi (Capital)">Tbilisi (Capital)</option>
                            <option value="Dire Dawa">Dire Dawa</option>
                            <option value="Bishkek (Province)">Bishkek (Province)</option>
                            <option value="Moscow (Oblast)">Moscow (Oblast)</option>
                            <option value="Enga (Province)">Enga (Province)</option>
                            <option value="Catanduanes">Catanduanes</option>
                            <option value="Parwan">Parwan</option>
                            <option value="Yala (District)">Yala (District)</option>
                            <option value="Urus-Martan (District)">Urus-Martan (District)</option>
                            <option value="Greater Skopje (Administrative division)">Greater Skopje (Administrative division)</option>
                            <option value="Kunar">Kunar</option>
                            <option value="Ogun">Ogun</option>
                            <option value="Sulawesi Tengah (Province)">Sulawesi Tengah (Province)</option>
                            <option value="Papua (Province)">Papua (Province)</option>
                            <option value="Chuy Province">Chuy Province</option>
                            <option value="North Maluku">North Maluku</option>
                            <option value="Relizane Province">Relizane Province</option>
                            <option value="Guangxi">Guangxi</option>
                            <option value="Gulf of Aden">Gulf of Aden</option>
                            <option value="Vallee du Bandama">Vallee du Bandama</option>
                            <option value="North Sulawesi (Province)">North Sulawesi (Province)</option>
                            <option value="Bali (Province)">Bali (Province)</option>
                            <option value="Chlef (Region)">Chlef (Region)</option>
                            <option value="Moscow Federal City">Moscow Federal City</option>
                            <option value="Pattani province">Pattani province</option>
                            <option value="Plateau">Plateau</option>
                            <option value="Kwara">Kwara</option>
                            <option value="North Ossetia–Alania">North Ossetia–Alania</option>
                            <option value="Ashgabat">Ashgabat</option>
                            <option value="Atsabe (District)">Atsabe (District)</option>
                            <option value="Veracruz (State)">Veracruz (State)</option>
                            <option value="Chiquimula (Department)">Chiquimula (Department)</option>
                            <option value="North Moravia (Province)">North Moravia (Province)</option>
                            <option value="Guatemala (Department)">Guatemala (Department)</option>
                            <option value="Aichi (Prefecture)">Aichi (Prefecture)</option>
                            <option value="Escuintla (Department)">Escuintla (Department)</option>
                            <option value="Voronezh (Oblast)">Voronezh (Oblast)</option>
                            <option value="Illizi">Illizi</option>
                            <option value="Tamanghasset">Tamanghasset</option>
                            <option value="Louangphrabang (District)">Louangphrabang (District)</option>
                            <option value="Belgrade (Autonomous Territory)">Belgrade (Autonomous Territory)</option>
                            <option value="Imo">Imo</option>
                            <option value="Malaita (Province)">Malaita (Province)</option>
                            <option value="Struga municipality">Struga municipality</option>
                            <option value="Auckland (Territory)">Auckland (Territory)</option>
                            <option value="Ouargla (Province)">Ouargla (Province)</option>
                            <option value="Wellington (Region)">Wellington (Region)</option>
                            <option value="Maliana (District)">Maliana (District)</option>
                            <option value="Nimroz">Nimroz</option>
                            <option value="Grand Gedeh">Grand Gedeh</option>
                            <option value="Najaf">Najaf</option>
                            <option value="Sumatra (Province)">Sumatra (Province)</option>
                            <option value="Uruzgan">Uruzgan</option>
                            <option value="Zabul">Zabul</option>
                            <option value="Pec (District)">Pec (District)</option>
                            <option value="Osh (Oblast)">Osh (Oblast)</option>
                            <option value="Bago (Division)">Bago (Division)</option>
                            <option value="Greater Casablanca (Region)">Greater Casablanca (Region)</option>
                            <option value="Logar">Logar</option>
                            <option value="Jakarta (Special Territory Province)">Jakarta (Special Territory Province)</option>
                            <option value="Volga (District)">Volga (District)</option>
                            <option value="Wassec (Region)">Wassec (Region)</option>
                            <option value="Ingushetia (Province)">Ingushetia (Province)</option>
                            <option value="Jizan">Jizan</option>
                            <option value="Medveda (District)">Medveda (District)</option>
                            <option value="Asmara">Asmara</option>
                            <option value="Artibonite (Department)">Artibonite (Department)</option>
                            <option value="Chkhorotsku (District)">Chkhorotsku (District)</option>
                            <option value="Jeola (Province)">Jeola (Province)</option>
                            <option value="Oum el-Bouaghi">Oum el-Bouaghi</option>
                            <option value="Babil">Babil</option>
                            <option value="Farah">Farah</option>
                            <option value="Ajaria (Autonomous Republic)">Ajaria (Autonomous Republic)</option>
                            <option value="Samangan">Samangan</option>
                            <option value="Ali Sabieh">Ali Sabieh</option>
                            <option value="Henan (Province)">Henan (Province)</option>
                            <option value="Centre (Department)">Centre (Department)</option>
                            <option value="Chechnya (Federal District)">Chechnya (Federal District)</option>
                            <option value="Siberia (Oblast)">Siberia (Oblast)</option>
                            <option value="Khanty-Mansi (Autonomous Okrug)">Khanty-Mansi (Autonomous Okrug)</option>
                            <option value="Port-au-Prince (Capital City)">Port-au-Prince (Capital City)</option>
                            <option value="Kogi">Kogi</option>
                            <option value="Tainan">Tainan</option>
                            <option value="Zanzibar West">Zanzibar West</option>
                            <option value="Tashkent (Province)">Tashkent (Province)</option>
                            <option value="Paktika">Paktika</option>
                            <option value="Madinah">Madinah</option>
                            <option value="Nuristan">Nuristan</option>
                            <option value="Ambon (Province)">Ambon (Province)</option>
                            <option value="El Aissaoui (Municipality)">El Aissaoui (Municipality)</option>
                            <option value="Baghlan">Baghlan</option>
                            <option value="Laghman">Laghman</option>
                            <option value="South Ossetia (Republic)">South Ossetia (Republic)</option>
                            <option value="Songkla (Province)">Songkla (Province)</option>
                            <option value="Rostov">Rostov</option>
                            <option value="Tula">Tula</option>
                            <option value="Qassim">Qassim</option>
                            <option value="Karachayevo-Cherkessia (Republic)">Karachayevo-Cherkessia (Republic)</option>
                            <option value="Guatemala City (Capital District)">Guatemala City (Capital District)</option>
                            <option value="Santa Rosa (Department)">Santa Rosa (Department)</option>
                            <option value="Nouakchott">Nouakchott</option>
                            <option value="South Sinai">South Sinai</option>
                            <option value="Borno">Borno</option>
                            <option value="North Darfur">North Darfur</option>
                            <option value="Taipei (County)">Taipei (County)</option>
                            <option value="Cortés (Department)">Cortés (Department)</option>
                            <option value="Krasnodar (Federal Subject)">Krasnodar (Federal Subject)</option>
                            <option value="Kachin (State)">Kachin (State)</option>
                            <option value="Shida Kartlie (Region)">Shida Kartlie (Region)</option>
                            <option value="Ituri">Ituri</option>
                            <option value="Chuy (Province)">Chuy (Province)</option>
                            <option value="Kosovska Mitrovica (District)">Kosovska Mitrovica (District)</option>
                            <option value="Prishtine (Municipality)">Prishtine (Municipality)</option>
                            <option value="Samtskhe-Javakheti (Province)">Samtskhe-Javakheti (Province)</option>
                            <option value="Yangon (State)">Yangon (State)</option>
                            <option value="Dagestan (Oblast)">Dagestan (Oblast)</option>
                            <option value="Karachay-Cherkessia (Autonomous Republic)">Karachay-Cherkessia (Autonomous Republic)
                            </option>
                            <option value="Saada">Saada</option>
                            <option value="Kayah (State)">Kayah (State)</option>
                            <option value="Mandalay (Division)">Mandalay (Division)</option>
                            <option value="Cairo (Governorate)">Cairo (Governorate)</option>
                            <option value="Bauchi">Bauchi</option>
                            <option value="Rayong (Province)">Rayong (Province)</option>
                            <option value="Kartli (Province)">Kartli (Province)</option>
                            <option value="Dili ( District )">Dili ( District )</option>
                            <option value="Andijon (Province)">Andijon (Province)</option>
                            <option value="Sokoto">Sokoto</option>
                            <option value="Khenchela (Province)">Khenchela (Province)</option>
                            <option value="Kabardino-Balkariya (Republic)">Kabardino-Balkariya (Republic)</option>
                            <option value="South Ossetia (Autonomous Province)">South Ossetia (Autonomous Province)</option>
                            <option value="Yarang ( District )">Yarang ( District )</option>
                            <option value="Sungai Padi ( District )">Sungai Padi ( District )</option>
                            <option value="Yaha ( District )">Yaha ( District )</option>
                            <option value="Tskhinvali (Region)">Tskhinvali (Region)</option>
                            <option value="Moscow (Region)">Moscow (Region)</option>
                            <option value="(Region) of Republican Subordination (Province)">(Region) of Republican Subordination
                                (Province)</option>
                            <option value="Sulawesi (Island)">Sulawesi (Island)</option>
                            <option value="Acapulco">Acapulco</option>
                            <option value="Port of Spain (Municipality)">Port of Spain (Municipality)</option>
                            <option value="Yala Muang  District">Yala Muang District</option>
                            <option value="Daykundi">Daykundi</option>
                            <option value="South Sinai Governorate">South Sinai Governorate</option>
                            <option value="Republic of Degastan">Republic of Degastan</option>
                            <option value="Urals (Federal  District )">Urals (Federal District )</option>
                            <option value="Port-of-Spain City Corporation">Port-of-Spain City Corporation</option>
                            <option value="Aqaba">Aqaba</option>
                            <option value="Khenchela">Khenchela</option>
                            <option value="Ghardaïa">Ghardaïa</option>
                            <option value="Zagreb (County)">Zagreb (County)</option>
                            <option value="Shida Kartli (Region)">Shida Kartli (Region)</option>
                            <option value="Pcinja ( District )">Pcinja ( District )</option>
                            <option value="Gnjilane (Region)">Gnjilane (Region)</option>
                            <option value="Osh (Province)">Osh (Province)</option>
                            <option value="Ra Ngae ( District )">Ra Ngae ( District )</option>
                            <option value="Bicol">Bicol</option>
                            <option value="Bangkok ( District )">Bangkok ( District )</option>
                            <option value="Arkhangelsk (Oblast)">Arkhangelsk (Oblast)</option>
                            <option value="Lezhe (County)">Lezhe (County)</option>
                            <option value="Su-ngai Kolok  District Su-ngai Kolok">Su-ngai Kolok District Su-ngai Kolok</option>
                            <option value="Cacheu">Cacheu</option>
                            <option value="Krasnoyarsk (Krai)">Krasnoyarsk (Krai)</option>
                            <option value="Tyumen (Oblast)">Tyumen (Oblast)</option>
                            <option value="Ghardaia (Province)">Ghardaia (Province)</option>
                            <option value="Tamaulipas (State)">Tamaulipas (State)</option>
                            <option value="Ouaddai">Ouaddai</option>
                            <option value="Ombella-M\Poko">Ombella-M\Poko</option>
                            <option value="Raska ( District )">Raska ( District )</option>
                            <option value="Guera">Guera</option>
                            <option value="Sa Kaeo (Province)">Sa Kaeo (Province)</option>
                            <option value="Zabaykalsky (Krai)">Zabaykalsky (Krai)</option>
                            <option value="Alexandria (Governorate)">Alexandria (Governorate)</option>
                            <option value="South Sinai (Governorate)">South Sinai (Governorate)</option>
                            <option value="Kapisa">Kapisa</option>
                            <option value="Mayo (district), Ban Dan (village)">Mayo (district), Ban Dan (village)</option>
                            <option value="Yala Province">Yala Province</option>
                            <option value="Narathiwat Province">Narathiwat Province</option>
                            <option value="Ingushetiya (Republic)">Ingushetiya (Republic)</option>
                            <option value="JalAl-Abad">JalAl-Abad</option>
                            <option value="Nabatiyeh">Nabatiyeh</option>
                            <option value="South Ossetia">South Ossetia</option>
                            <option value="Abkhazia (Autonomous Region)">Abkhazia (Autonomous Region)</option>
                            <option value="Tak Bai ( District )">Tak Bai ( District )</option>
                            <option value="Caucasus (Region)">Caucasus (Region)</option>
                            <option value="North Caucasus (Region)">North Caucasus (Region)</option>
                            <option value="Aomar (Region)">Aomar (Region)</option>
                            <option value="Dniester (Region)">Dniester (Region)</option>
                            <option value="Kurchaloyevsky">Kurchaloyevsky</option>
                            <option value="Malgobeksky">Malgobeksky</option>
                            <option value="Quintana">Quintana</option>
                            <option value="Sulawesi">Sulawesi</option>
                            <option value="Kamphaeng Phet (Province)">Kamphaeng Phet (Province)</option>
                            <option value="North-Caucasian (Region)">North-Caucasian (Region)</option>
                            <option value="Pattani, Narathiwat, and Yala">Pattani, Narathiwat, and Yala</option>
                            <option value="Chukha">Chukha</option>
                            <option value="Wadi Fira">Wadi Fira</option>
                            <option value="Yala ( District )">Yala ( District )</option>
                            <option value="Chaing Mai (Province)">Chaing Mai (Province)</option>
                            <option value="Nabuel (Region)">Nabuel (Region)</option>
                            <option value="Jijel (Region)">Jijel (Region)</option>
                            <option value="Tamalous (Region)">Tamalous (Region)</option>
                            <option value="North Catobato">North Catobato</option>
                            <option value="Lacs">Lacs</option>
                            <option value="Inguisha">Inguisha</option>
                            <option value="Than To">Than To</option>
                            <option value="Rangae">Rangae</option>
                            <option value="Ennedi-Ouest">Ennedi-Ouest</option>
                            <option value="Afar">Afar</option>
                            <option value="Nuevo Leon (State)">Nuevo Leon (State)</option>
                            <option value="Narathiwat (Provice)">Narathiwat (Provice)</option>
                            <option value="Mitrovice (Municipality)">Mitrovice (Municipality)</option>
                            <option value="Sungai Kolok">Sungai Kolok</option>
                            <option value="Havana (Province)">Havana (Province)</option>
                            <option value="Sultan Kudurat">Sultan Kudurat</option>
                            <option value="Kidal">Kidal</option>
                            <option value="Viqueque (District)">Viqueque (District)</option>
                            <option value="Pattani (Provience)">Pattani (Provience)</option>
                            <option value="Republic of Dagestan">Republic of Dagestan</option>
                            <option value="Ouham-Pende">Ouham-Pende</option>
                            <option value="Agadez (Region)">Agadez (Region)</option>
                            <option value="Agadez (Department)">Agadez (Department)</option>
                            <option value="Queretaro (State)">Queretaro (State)</option>
                            <option value="Meknes-Tafilalet (Region)">Meknes-Tafilalet (Region)</option>
                            <option value="Amdjoudh Massif">Amdjoudh Massif</option>
                            <option value="Novgorod Oblast">Novgorod Oblast</option>
                            <option value="South East St. Andrew">South East St. Andrew</option>
                            <option value="Veracruz (State) and Tlaxcala (State)">Veracruz (State) and Tlaxcala (State)</option>
                            <option value="Yala  District">Yala District</option>
                            <option value="Gash Barka">Gash Barka</option>
                            <option value="Trat (Province)">Trat (Province)</option>
                            <option value="Roztochia Upland">Roztochia Upland</option>
                            <option value="Jawzjan">Jawzjan</option>
                            <option value="Illizi (Province)">Illizi (Province)</option>
                            <option value="South Highlands (Province)">South Highlands (Province)</option>
                            <option value="Pattani ( District )">Pattani ( District )</option>
                            <option value="Southwest">Southwest</option>
                            <option value="Kosovo-Metohija (Province)">Kosovo-Metohija (Province)</option>
                            <option value="Cababobo">Cababobo</option>
                            <option value="Chelyabinsk (Oblast)">Chelyabinsk (Oblast)</option>
                            <option value="Sool">Sool</option>
                            <option value="Warap">Warap</option>
                            <option value="Kosovo and Metojia (Autonomous Province)">Kosovo and Metojia (Autonomous Province)</option>
                            <option value="Niamey (Administrative Region)">Niamey (Administrative Region)</option>
                            <option value="Ingushetia (State)">Ingushetia (State)</option>
                            <option value="Upper North Province">Upper North Province</option>
                            <option value="Dagestan (Region)">Dagestan (Region)</option>
                            <option value="Yala Province/Muang District">Yala Province/Muang District</option>
                            <option value="Tanout Department">Tanout Department</option>
                            <option value="Pattani and Yala Provinces">Pattani and Yala Provinces</option>
                            <option value="Grozny">Grozny</option>
                            <option value="Agadez Department">Agadez Department</option>
                            <option value="Pattani, Narathiwat, and Yala Provinces">Pattani, Narathiwat, and Yala Provinces</option>
                            <option value="Kabylie Region">Kabylie Region</option>
                            <option value="Yala and Pattani Provinces">Yala and Pattani Provinces</option>
                            <option value="Voronezh Oblast">Voronezh Oblast</option>
                            <option value="Cuyuni-Mazaruni">Cuyuni-Mazaruni</option>
                            <option value="Istria (County)">Istria (County)</option>
                            <option value="Gabes">Gabes</option>
                            <option value="(Republic of) Ingushetia">(Republic of) Ingushetia</option>
                            <option value="Shiselweni">Shiselweni</option>
                            <option value="Pattini (Province)">Pattini (Province)</option>
                            <option value="Jijil (Province)">Jijil (Province)</option>
                            <option value="Niamey Capital District">Niamey Capital District</option>
                            <option value="Smolensk Oblast (Administrative Region)">Smolensk Oblast (Administrative Region)</option>
                            <option value="Daghestan (Republic)">Daghestan (Republic)</option>
                            <option value="Chechen Republic">Chechen Republic</option>
                            <option value="South Ossetia (Region)">South Ossetia (Region)</option>
                            <option value="Sahara">Sahara</option>
                            <option value="Hungary">Hungary</option>
                            <option value="Yangon (Rangoon)">Yangon (Rangoon)</option>
                            <option value="Kabardino Balkariya">Kabardino Balkariya</option>
                            <option value="West Equatoria">West Equatoria</option>
                            <option value="Leposaviq">Leposaviq</option>
                            <option value="Segou">Segou</option>
                            <option value="Mudug">Mudug</option>
                            <option value="Setif (Stif)">Setif (Stif)</option>
                            <option value="Orahovo">Orahovo</option>
                            <option value="Karachayevo Cherkesiya Republic">Karachayevo Cherkesiya Republic</option>
                            <option value="Bordj Bou Arreridj">Bordj Bou Arreridj</option>
                            <option value="Abia">Abia</option>
                            <option value="Praha">Praha</option>
                            <option value="Stavropolye Region">Stavropolye Region</option>
                            <option value="Kalimantan Timur">Kalimantan Timur</option>
                            <option value="Arunachal Pradesh">Arunachal Pradesh</option>
                            <option value="Kalinga-Apayao">Kalinga-Apayao</option>
                            <option value="Kosovsko Mitrovica">Kosovsko Mitrovica</option>
                            <option value="Mitrovice">Mitrovice</option>
                            <option value="Jijel Province">Jijel Province</option>
                            <option value="M\sila Province">M\sila Province</option>
                            <option value="Benue">Benue</option>
                            <option value="Samegrelo and Zemo Svaneti">Samegrelo and Zemo Svaneti</option>
                            <option value="Kabardino-Balkariya">Kabardino-Balkariya</option>
                            <option value="Boumerdes Province">Boumerdes Province</option>
                            <option value="Bago (Pegu)">Bago (Pegu)</option>
                            <option value="Ebonyi">Ebonyi</option>
                            <option value="Kawthulei (Karen/Kayin)">Kawthulei (Karen/Kayin)</option>
                            <option value="Krasnodar Krai">Krasnodar Krai</option>
                            <option value="Propinsi Sulawesi Tengah">Propinsi Sulawesi Tengah</option>
                            <option value="Sulawesi Tengah (Central Sulawesi)">Sulawesi Tengah (Central Sulawesi)</option>
                            <option value="Karen">Karen</option>
                            <option value="Nazranovsky">Nazranovsky</option>
                            <option value="Russian Republic">Russian Republic</option>
                            <option value="Skra">Skra</option>
                            <option value="Lubombo">Lubombo</option>
                            <option value="Karlovacka">Karlovacka</option>
                            <option value="Nonthaburi">Nonthaburi</option>
                            <option value="Skikda Province">Skikda Province</option>
                            <option value="Westmeath">Westmeath</option>
                            <option value="Tebessa Province">Tebessa Province</option>
                            <option value="Kufrah">Kufrah</option>
                            <option value="Tizi Ouzou Province">Tizi Ouzou Province</option>
                            <option value="Batna Province">Batna Province</option>
                            <option value="Heilongjiang">Heilongjiang</option>
                            <option value="Hunan">Hunan</option>
                            <option value="Yangon Division">Yangon Division</option>
                            <option value="Vakaga">Vakaga</option>
                            <option value="Kedah">Kedah</option>
                            <option value="Shikoku">Shikoku</option>
                            <option value="Bandundu">Bandundu</option>
                            <option value="Phetchabun">Phetchabun</option>
                            <option value="Kasai Oriental">Kasai Oriental</option>
                            <option value="Central Darfur">Central Darfur</option>
                            <option value="Shamal Sina">Shamal Sina</option>
                            <option value="T\bilisi">T\bilisi</option>
                            <option value="Samara">Samara</option>
                            <option value="Hong Kong Special Administrative Region (hksar)">Hong Kong Special Administrative Region
                                (hksar)</option>
                            <option value="Sarpang">Sarpang</option>
                            <option value="Tbessa">Tbessa</option>
                            <option value="Bioko Norte">Bioko Norte</option>
                            <option value="Al Qahirah (Cairo)">Al Qahirah (Cairo)</option>
                            <option value="Fukuoka">Fukuoka</option>
                            <option value="Pest">Pest</option>
                            <option value="Béjaïa Province">Béjaïa Province</option>
                            <option value="Pathum thani">Pathum thani</option>
                            <option value="Zubin Potok">Zubin Potok</option>
                            <option value="Lakhdaria Province">Lakhdaria Province</option>
                            <option value="Leninabad">Leninabad</option>
                            <option value="Haut Congo">Haut Congo</option>
                            <option value="Constantine Province">Constantine Province</option>
                            <option value="Biskra Province">Biskra Province</option>
                            <option value="Andijon">Andijon</option>
                            <option value="Mtskheta-Mtianeti">Mtskheta-Mtianeti</option>
                            <option value="Zugdidi">Zugdidi</option>
                            <option value="Bordj Bou Arreridj Province">Bordj Bou Arreridj Province</option>
                            <option value="Jalilabad">Jalilabad</option>
                            <option value="sakon nakhon">sakon nakhon</option>
                            <option value="Khenchela Province">Khenchela Province</option>
                            <option value="Dalmatia">Dalmatia</option>
                            <option value="Mascara Province">Mascara Province</option>
                            <option value="Shan State">Shan State</option>
                            <option value="Tissemsilt Province">Tissemsilt Province</option>
                            <option value="sulawesi utara">sulawesi utara</option>
                            <option value="Setif Province">Setif Province</option>
                            <option value="Iligan">Iligan</option>
                            <option value="Haut-Mbomou">Haut-Mbomou</option>
                            <option value="Bamingui-Bangoran">Bamingui-Bangoran</option>
                            <option value="Tver">Tver</option>
                            <option value="Sidi Bel Abbès Province">Sidi Bel Abbès Province</option>
                            <option value="Oran Province">Oran Province</option>
                            <option value="Mbomou">Mbomou</option>
                            <option value="Rostov (Rostovskaya) Oblast">Rostov (Rostovskaya) Oblast</option>
                            <option value="samut prakan">samut prakan</option>
                            <option value="khon kaen">khon kaen</option>
                            <option value="Gori">Gori</option>
                            <option value="phayao">phayao</option>
                            <option value="Equateur">Equateur</option>
                            <option value="Ayutthaya">Ayutthaya</option>
                            <option value="Pegu">Pegu</option>
                            <option value="Haute-Kotto">Haute-Kotto</option>
                            <option value="Mukdahan">Mukdahan</option>
                            <option value="Concepcion">Concepcion</option>
                            <option value="Darfur">Darfur</option>
                            <option value="Sangre Grande Regional Corporation">Sangre Grande Regional Corporation</option>
                            <option value="Oryol Oblast">Oryol Oblast</option>
                            <option value="Fujairah">Fujairah</option>
                            <option value="Hodh Ech Chargui">Hodh Ech Chargui</option>
                            <option value="Canindeyu">Canindeyu</option>
                            <option value="Soghd">Soghd</option>
                            <option value="Lodz">Lodz</option>
                            <option value="Paktika Province">Paktika Province</option>
                            <option value="Zolsky">Zolsky</option>
                            <option value="Bir H\mam">Bir H\mam</option>
                            <option value="Krasnodarsky">Krasnodarsky</option>
                            <option value="Haut-Sassandra">Haut-Sassandra</option>
                            <option value="Elbrussky">Elbrussky</option>
                            <option value="Al Iskandariyah (Alexandria)">Al Iskandariyah (Alexandria)</option>
                            <option value="Akershus">Akershus</option>
                            <option value="Gombe">Gombe</option>
                            <option value="Donetsk Oblast">Donetsk Oblast</option>
                            <option value="Moscow Oblast">Moscow Oblast</option>
                            <option value="Caazapa">Caazapa</option>
                            <option value="Traza">Traza</option>
                            <option value="Niger">Niger</option>
                            <option value="Wilaya de Djelfa">Wilaya de Djelfa</option>
                            <option value="Jawa Barat (West Java)">Jawa Barat (West Java)</option>
                            <option value="Romblon">Romblon</option>
                            <option value="Volgograd Oblast">Volgograd Oblast</option>
                            <option value="Alger">Alger</option>
                            <option value="Marrakech-Tensift-El Haouz">Marrakech-Tensift-El Haouz</option>
                            <option value="Java">Java</option>
                            <option value="Kebbi">Kebbi</option>
                            <option value="Tataouine">Tataouine</option>
                            <option value="Aktobe">Aktobe</option>
                            <option value="Siliana">Siliana</option>
                            <option value="Yobe">Yobe</option>
                            <option value="Najran">Najran</option>
                            <option value="Gezira">Gezira</option>
                            <option value="Samara Oblast">Samara Oblast</option>
                            <option value="Sumatera Selatan">Sumatera Selatan</option>
                            <option value="México">México</option>
                            <option value="Mindanao Island">Mindanao Island</option>
                            <option value="Deir ez-Zor">Deir ez-Zor</option>
                            <option value="Zaghouan">Zaghouan</option>
                            <option value="Buskerud">Buskerud</option>
                            <option value="Mistrata">Mistrata</option>
                            <option value="Jawa Tengah (Central Java)">Jawa Tengah (Central Java)</option>
                            <option value="Jubaland">Jubaland</option>
                            <option value="Sistan va Baluchestan">Sistan va Baluchestan</option>
                            <option value="Tindouf">Tindouf</option>
                            <option value="Atyrau">Atyrau</option>
                            <option value="Karen State">Karen State</option>
                            <option value="Papua Province">Papua Province</option>
                            <option value="Quezon Province">Quezon Province</option>
                            <option value="Almaty Province">Almaty Province</option>
                            <option value="Yaroslavl Oblast">Yaroslavl Oblast</option>
                            <option value="Idlib Region">Idlib Region</option>
                            <option value="Davao Region">Davao Region</option>
                            <option value="North Sinai Governorate">North Sinai Governorate</option>
                            <option value="Jambyl Province">Jambyl Province</option>
                            <option value="Kabarda-Balkaria">Kabarda-Balkaria</option>
                            <option value="Ingushetia Republic">Ingushetia Republic</option>
                            <option value="Garissa">Garissa</option>
                            <option value="Irrawaddy Region">Irrawaddy Region</option>
                            <option value="Sedhiou">Sedhiou</option>
                            <option value="District of Kosovska Mitrovica">District of Kosovska Mitrovica</option>
                            <option value="Bangkok Province">Bangkok Province</option>
                            <option value="Central District">Central District</option>
                            <option value="North Caucasian Federal District">North Caucasian Federal District</option>
                            <option value="Sinai">Sinai</option>
                            <option value="Jigawa">Jigawa</option>
                            <option value="Poltava">Poltava</option>
                            <option value="White Nile">White Nile</option>
                            <option value="Mahwit">Mahwit</option>
                            <option value="Hofudhborgarsvaedhi">Hofudhborgarsvaedhi</option>
                            <option value="Tawi-Tawi">Tawi-Tawi</option>
                            <option value="Marahoue">Marahoue</option>
                            <option value="Al Qadisiyah">Al Qadisiyah</option>
                            <option value="West Kazakhstan">West Kazakhstan</option>
                            <option value="Tartus">Tartus</option>
                            <option value="Nasarawa">Nasarawa</option>
                            <option value="Ciudad de Buenos Aires">Ciudad de Buenos Aires</option>
                            <option value="Ivano-Frankivsk">Ivano-Frankivsk</option>
                            <option value="Southern and Eastern Serbia">Southern and Eastern Serbia</option>
                            <option value="Gomel">Gomel</option>
                            <option value="Misrata">Misrata</option>
                            <option value="Ouargla">Ouargla</option>
                            <option value="Blagoevgrad">Blagoevgrad</option>
                            <option value="Clare">Clare</option>
                            <option value="Wahat">Wahat</option>
                            <option value="Putrajaya">Putrajaya</option>
                            <option value="Riau Islands">Riau Islands</option>
                            <option value="Moyen-Comoe">Moyen-Comoe</option>
                            <option value="Analamanga">Analamanga</option>
                            <option value="Panjsher">Panjsher</option>
                            <option value="Sud-Comoe">Sud-Comoe</option>
                            <option value="Manouba">Manouba</option>
                            <option value="Sistan and Baluchestan">Sistan and Baluchestan</option>
                            <option value="Raqqah">Raqqah</option>
                            <option value="Grodno">Grodno</option>
                            <option value="Phangnga">Phangnga</option>
                            <option value="Vitebsk">Vitebsk</option>
                            <option value="San Pedro">San Pedro</option>
                            <option value="Kayes">Kayes</option>
                            <option value="Kemo">Kemo</option>
                            <option value="Nouvelle-Aquitaine">Nouvelle-Aquitaine</option>
                            <option value="Lattakia">Lattakia</option>
                            <option value="Marj">Marj</option>
                            <option value="Sabha">Sabha</option>
                            <option value="Kufra">Kufra</option>
                            <option value="South Bohemia">South Bohemia</option>
                            <option value="Butnan">Butnan</option>
                            <option value="Lviv">Lviv</option>
                            <option value="Thessaly">Thessaly</option>
                            <option value="Mandera">Mandera</option>
                            <option value="Ouham">Ouham</option>
                            <option value="Bungoma">Bungoma</option>
                            <option value="Mombasa">Mombasa</option>
                            <option value="Wajir">Wajir</option>
                            <option value="Sirte">Sirte</option>
                            <option value="Kilifi">Kilifi</option>
                            <option value="Nakuru">Nakuru</option>
                            <option value="Kurdufan">Kurdufan</option>
                            <option value="Nana-Grebizi">Nana-Grebizi</option>
                            <option value="Selangor">Selangor</option>
                            <option value="Kef">Kef</option>
                            <option value="Lipetsk">Lipetsk</option>
                            <option value="Isiolo">Isiolo</option>
                            <option value="Zawiya">Zawiya</option>
                            <option value="Aqmola">Aqmola</option>
                            <option value="Matruh">Matruh</option>
                            <option value="Donetsk">Donetsk</option>
                            <option value="Dakahlia">Dakahlia</option>
                            <option value="Ben Arous">Ben Arous</option>
                            <option value="Banten">Banten</option>
                            <option value="Berane">Berane</option>
                            <option value="Kiambu">Kiambu</option>
                            <option value="Free State">Free State</option>
                            <option value="Lekoumou">Lekoumou</option>
                            <option value="Alborz">Alborz</option>
                            <option value="Longford">Longford</option>
                            <option value="Sumy">Sumy</option>
                            <option value="Tobago">Tobago</option>
                            <option value="Point Fortin">Point Fortin</option>
                            <option value="Jendouba">Jendouba</option>
                            <option value="Sidi Bou Zid">Sidi Bou Zid</option>
                            <option value="As Suwayda">As Suwayda</option>
                            <option value="Benishangul-Gumuz">Benishangul-Gumuz</option>
                            <option value="Shanxi">Shanxi</option>
                            <option value="Nuqat Al Khams">Nuqat Al Khams</option>
                            <option value="Gracias a Dios">Gracias a Dios</option>
                            <option value="Turkana">Turkana</option>
                            <option value="Kwale">Kwale</option>
                            <option value="Kafr El-Sheikh">Kafr El-Sheikh</option>
                            <option value="Marsabit">Marsabit</option>
                            <option value="Murzuq">Murzuq</option>
                            <option value="Guizhou">Guizhou</option>
                            <option value="Nana-Mambere">Nana-Mambere</option>
                            <option value="Central Bohemia">Central Bohemia</option>
                            <option value="Phetchaburi">Phetchaburi</option>
                            <option value="Poitou-Charentes">Poitou-Charentes</option>
                            <option value="Jabal Al Gharbi">Jabal Al Gharbi</option>
                            <option value="Puducherry">Puducherry</option>
                            <option value="Nan">Nan</option>
                            <option value="Sakhalin">Sakhalin</option>
                            <option value="Rayong">Rayong</option>
                            <option value="Mafraq">Mafraq</option>
                            <option value="Kherson">Kherson</option>
                            <option value="Wadi Al Hayaa">Wadi Al Hayaa</option>
                            <option value="Trat">Trat</option>
                            <option value="Bas-Sassandra">Bas-Sassandra</option>
                            <option value="Zanzibar South">Zanzibar South</option>
                            <option value="Mary">Mary</option>
                            <option value="Haa Dhaalu Atoll">Haa Dhaalu Atoll</option>
                            <option value="Nigde">Nigde</option>
                            <option value="Lobaye">Lobaye</option>
                            <option value="Chuy">Chuy</option>
                            <option value="Lamu">Lamu</option>
                            <option value="Western Equatoria">Western Equatoria</option>
                            <option value="Cherkasy">Cherkasy</option>
                            <option value="Luhansk">Luhansk</option>
                            <option value="Ouaka">Ouaka</option>
                            <option value="Davao Occidental">Davao Occidental</option>
                            <option value="Jazan">Jazan</option>
                            <option value="Nordhurland Eystra">Nordhurland Eystra</option>
                            <option value="Mwanza">Mwanza</option>
                            <option value="Haut-Uele">Haut-Uele</option>
                            <option value="Negeri Sembilan">Negeri Sembilan</option>
                            <option value="Nyandarua">Nyandarua</option>
                            <option value="New Brunswick">New Brunswick</option>
                            <option value="Gjakova">Gjakova</option>
                            <option value="Zaporizhzhya">Zaporizhzhya</option>
                            <option value="Meru">Meru</option>
                            <option value="Gedarif">Gedarif</option>
                            <option value="Tana River">Tana River</option>
                            <option value="Northern Borders">Northern Borders</option>
                            <option value="Qinghai">Qinghai</option>
                            <option value="Kairouan">Kairouan</option>
                            <option value="Medenine">Medenine</option>
                            <option value="Amambay">Amambay</option>
                            <option value="Rivne">Rivne</option>
                            <option value="West Nusa Tenggara">West Nusa Tenggara</option>
                            <option value="Chernihiv">Chernihiv</option>
                            <option value="Usti nad Labem">Usti nad Labem</option>
                            <option value="Issyk Kul">Issyk Kul</option>
                            <option value="Jafara">Jafara</option>
                            <option value="South Sumatra">South Sumatra</option>
                            <option value="Kyivska">Kyivska</option>
                            <option value="Distrito Nacional">Distrito Nacional</option>
                            <option value="Mayo">Mayo</option>
                            <option value="Ghat">Ghat</option>
                            <option value="Bizerte">Bizerte</option>
                            <option value="Quba">Quba</option>
                            <option value="Zhytomyr">Zhytomyr</option>
                            <option value="Indre-et-loire">Indre-et-loire</option>
                            <option value="Sangha-Mbaere">Sangha-Mbaere</option>
                            <option value="Burgundy">Burgundy</option>
                            <option value="Mykolayiv">Mykolayiv</option>
                            <option value="Jufra">Jufra</option>
                            <option value="Telangana">Telangana</option>
                            <option value="Siquijor">Siquijor</option>
                            <option value="Pays de la Loire">Pays de la Loire</option>
                            <option value="Chernivtsi">Chernivtsi</option>
                            <option value="Centre-Val de Loire">Centre-Val de Loire</option>
                            <option value="Sarawak">Sarawak</option>
                            <option value="Lac">Lac</option>
                            <option value="Tanga">Tanga</option>
                            <option value="Kagera">Kagera</option>
                            <option value="Sahel">Sahel</option>
                            <option value="Samburu">Samburu</option>
                            <option value="Kolasin">Kolasin</option>
                            <option value="Tasmania">Tasmania</option>
                            <option value="Sikasso">Sikasso</option>
                            <option value="Vasterbotten">Vasterbotten</option>
                            <option value="South Kurdufan">South Kurdufan</option>
                            <option value="Koulikoro">Koulikoro</option>
                            <option value="Zebbug">Zebbug</option>
                            <option value="Zakarpattia">Zakarpattia</option>
                            <option value="Basse-Kotto">Basse-Kotto</option>
                            <option value="Jonkoping">Jonkoping</option>
                            <option value="Centre-Nord">Centre-Nord</option>
                            <option value="Volyn">Volyn</option>
                            <option value="Laane-Viru">Laane-Viru</option>
                            <option value="Norrbotten">Norrbotten</option>
                            <option value="Hail">Hail</option>
                            <option value="Giresun">Giresun</option>
                            <option value="Kaafu Atoll">Kaafu Atoll</option>
                            <option value="Gavleborg">Gavleborg</option>
                            <option value="Western Finland">Western Finland</option>
                            <option value="Hauts-Bassins">Hauts-Bassins</option>
                            <option value="Kronoberg">Kronoberg</option>
                            <option value="Halland">Halland</option>
                            <option value="Sodermanland">Sodermanland</option>
                            <option value="Kalmar">Kalmar</option>
                            <option value="Orebro">Orebro</option>
                            <option value="Sidi Bouzid">Sidi Bouzid</option>
                            <option value="Jubek">Jubek</option>
                            <option value="Kasai-Oriental">Kasai-Oriental</option>
                            <option value="Northern Ostrobothnia">Northern Ostrobothnia</option>
                            <option value="Satakunta">Satakunta</option>
                            <option value="Imatong">Imatong</option>
                            <option value="Pirkanmaa">Pirkanmaa</option>
                            <option value="Gbudwe">Gbudwe</option>
                            <option value="Paijanne Tavastia">Paijanne Tavastia</option>
                            <option value="Kirinyaga">Kirinyaga</option>
                            <option value="Southwest Finland">Southwest Finland</option>
                            <option value="Auvergne-Rhone-Alpes">Auvergne-Rhone-Alpes</option>
                            <option value="Alif Dhaalu">Alif Dhaalu</option>
                            <option value="Xaisomboun">Xaisomboun</option>
                            <option value="Galway">Galway</option>
                            <option value="Bas-Uele">Bas-Uele</option>
                            <option value="Central Finland">Central Finland</option>
                            <option value="Bay of Plenty">Bay of Plenty</option>
                            <option value="Louangphrabang">Louangphrabang</option>
                            <option value="Haut-Katanga">Haut-Katanga</option>
                            <option value="Paysandu">Paysandu</option>
                            <option value="Kirovohrad">Kirovohrad</option>
                            <option value="Kwango">Kwango</option>
                            <option value="Rumonge">Rumonge</option>
                            <option value="Vasternorrland">Vasternorrland</option>
                            <option value="Bio-Bio">Bio-Bio</option>
                            <option value="Yei River">Yei River</option>
                            <option value="Grand-Est">Grand-Est</option>
                            <option value="Vologda">Vologda</option>
                            <option value="Lower Silesia">Lower Silesia</option>
                            <option value="Masovia">Masovia</option>
                            <option value="Tanganyika">Tanganyika</option>
                            <option value="Occitanie">Occitanie</option>
                            <option value="Chumphon">Chumphon</option>
                            <option value="East Berbice-Corentyne">East Berbice-Corentyne</option>
                            <option value="Lori">Lori</option>
                            <option value="Balqa">Balqa</option>
                            <option value="Lol">Lol</option>
                            <option value="Wau">Wau</option>
                            <option value="Portuguesa">Portuguesa</option>
                            <option value="Trans-Nzoia">Trans-Nzoia</option>
                            <option value="Lualaba">Lualaba</option>
                            <option value="Normandy">Normandy</option>
                            <option value="Niassa">Niassa</option>
                            <option value="Alto Paraguay">Alto Paraguay</option>
                            <option value="Kasai-Central">Kasai-Central</option>
                            <option value="Altai">Altai</option>
                            <option value="Kisumu">Kisumu</option>
                            <option value="Nizhny Novgorod">Nizhny Novgorod</option>
                            <option value="Amadi">Amadi</option>
                            <option value="Hauts-de-France">Hauts-de-France</option>
                            <option value="Magway">Magway</option>
                            <option value="East Kalimantan">East Kalimantan</option>
                            <option value="West Kalimantan">West Kalimantan</option>
                            <option value="Morona Santiago">Morona Santiago</option>
                            <option value="Perlis">Perlis</option>
                            <option value="Gyeongsangbuk-do">Gyeongsangbuk-do</option>
                            <option value="Equator">Equator</option>
                            <option value="Karak">Karak</option>
                            <option value="Mongala">Mongala</option>
                            <option value="Kakamega">Kakamega</option>
                            <option value="Province 2">Province 2</option>
                            <option value="Northern Upper Nile">Northern Upper Nile</option>
                            <option value="Menaka">Menaka</option>
                            <option value="Province 3">Province 3</option>
                            <option value="Province 7">Province 7</option>
                            <option value="Greater Poland">Greater Poland</option>
                            <option value="Province 4">Province 4</option>
                            <option value="U.S. Virgin Islands">U.S. Virgin Islands</option>
                            <option value="Lomami">Lomami</option>
                            <option value="Pelagonia">Pelagonia</option>
                            <option value="Ismailli">Ismailli</option>
                            <option value="Kebili">Kebili</option>
                            <option value="Western Bieh">Western Bieh</option>
                            <option value="Southern Liech">Southern Liech</option>
                            <option value="Eastern Lakes">Eastern Lakes</option>
                            <option value="Sila">Sila</option>
                            <option value="Quirino">Quirino</option>
                            <option value="Terekeka">Terekeka</option>
                            <option value="Nayarit">Nayarit</option>
                            <option value="Baja California Sur">Baja California Sur</option>
                            <option value="South-West">South-West</option>
                            <option value="Kasai">Kasai</option>
                            <option value="Migori">Migori</option>
                            <option value="Central Upper Nile">Central Upper Nile</option>
                            <option value="Boma">Boma</option>
                            <option value="Dong Nai">Dong Nai</option>
                            <option value="Nalut">Nalut</option>
                            <option value="Pwani">Pwani</option>
                            <option value="Khabarovsk">Khabarovsk</option>
                            <option value="Ho Chi Minh City">Ho Chi Minh City</option>
                            <option value="Province 6">Province 6</option>
                            <option value="Province 5">Province 5</option>
                            <option value="Krasnoyarsk">Krasnoyarsk</option>
                            <option value="Laikipia">Laikipia</option>
                            <option value="Central Bohemian Region">Central Bohemian Region</option>
                            <option value="Jiangsu">Jiangsu</option>
                            <option value="Province 1">Province 1</option>
                            <option value="Kirov">Kirov</option>
                            <option value="Boucle du Mouhoun">Boucle du Mouhoun</option>
                            <option value="Varmland">Varmland</option>
                            <option value="Savanes">Savanes</option>
                            <option value="Jiwaka">Jiwaka</option>
                            <option value="New Ireland">New Ireland</option>
                            <option value="Siaya">Siaya</option>
                            <option value="Jamtland">Jamtland</option>
                            <option value="Sud-Bandama">Sud-Bandama</option>
                            <option value="Moravia-Silesia">Moravia-Silesia</option>
                            <option value="Kisii">Kisii</option>
                            <option value="North-West">North-West</option>
                            <option value="Tavastia Proper">Tavastia Proper</option>
                            <option value="Dodoma">Dodoma</option>
                            <option value="Homa Bay">Homa Bay</option>
                            <option value="Bourgogne-Franche-Comte">Bourgogne-Franche-Comte</option>
                            <option value="Eastern Bieh">Eastern Bieh</option>
                            <option value="Mosta">Mosta</option>
                            <option value="Carlow">Carlow</option>
                            <option value="Western Lakes">Western Lakes</option>
                            <option value="Vidzeme">Vidzeme</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="attacktype1_txt">Tip atac:</label>
                    </div>
                    <div class="filteritem">
                        <select id="attacktype1_txt" name="attacktype1_txt">
                            <option></option>
                            <option value="Assassination">Assassination</option>
                            <option value="Hostage Taking (Kidnapping)">Hostage Taking (Kidnapping)</option>
                            <option value="Bombing/Explosion">Bombing/Explosion</option>
                            <option value="Facility/Infrastructure Attack">Facility/Infrastructure Attack</option>
                            <option value="Armed Assault">Armed Assault</option>
                            <option value="Hijacking">Hijacking</option>
                            <option value="Unknown">Unknown</option>
                            <option value="Unarmed Assault">Unarmed Assault</option>
                            <option value="Other Crime Type">Other Crime Type</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="targtype1_txt">Tinta principala:</label>
                    </div>
                    <div class="filteritem">
                        <select id="targtype1_txt" name="targtype1_txt">
                            <option></option>
                            <option value="Private Citizens & Property">Private Citizens & Property</option>
                            <option value="Government (Diplomatic)">Government (Diplomatic)</option>
                            <option value="Journalists & Media">Journalists & Media</option>
                            <option value="Police">Police</option>
                            <option value="Utilities">Utilities</option>
                            <option value="Military">Military</option>
                            <option value="Government (General)">Government (General)</option>
                            <option value="Airports & Aircraft">Airports & Aircraft</option>
                            <option value="Business">Business</option>
                            <option value="Educational Institution">Educational Institution</option>
                            <option value="Violent Political Party">Violent Political Party</option>
                            <option value="Religious Figures/Institutions">Religious Figures/Institutions</option>
                            <option value="Unknown">Unknown</option>
                            <option value="Transportation">Transportation</option>
                            <option value="Tourists">Tourists</option>
                            <option value="NGO">NGO</option>
                            <option value="Telecommunication">Telecommunication</option>
                            <option value="Food or Water Supply">Food or Water Supply</option>
                            <option value="Facility/Infrastructure Attack">Facility/Infrastructure Attack</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="targsubtype1_txt">Tinta secundara:</label>
                    </div>
                    <div class="filteritem">
                        <select id="targsubtype1_txt" name="targsubtype1_txt">
                            <option></option>
                            <option value="Named Civilian">Named Civilian</option>
                            <option value="Diplomatic Personnel (outside of embassy, consulate)">Diplomatic Personnel (outside of
                                embassy, consulate)</option>
                            <option value="Radio Journalist/Staff/Facility">Radio Journalist/Staff/Facility</option>
                            <option value="Embassy/Consulate">Embassy/Consulate</option>
                            <option value="Police Building (headquarters, station, school)">Police Building (headquarters, station,
                                school)</option>
                            <option value="Police Security Forces/Officers">Police Security Forces/Officers</option>
                            <option value="Electricity">Electricity</option>
                            <option value="Military Recruiting Station/Academy">Military Recruiting Station/Academy</option>
                            <option value="Government Building/Facility/Office">Government Building/Facility/Office</option>
                            <option value="Military Barracks/Base/Headquarters/Checkpost">Military
                                Barracks/Base/Headquarters/Checkpost</option>
                            <option value="Aircraft (not at an airport)">Aircraft (not at an airport)</option>
                            <option value="Retail/Grocery/Bakery">Retail/Grocery/Bakery</option>
                            <option value="Military Personnel (soldiers, troops, officers, forces)">Military Personnel (soldiers,
                                troops, officers, forces)</option>
                            <option value="School/University/Educational Building">School/University/Educational Building</option>
                            <option value="Multinational Corporation">Multinational Corporation</option>
                            <option value="Party Office/Facility">Party Office/Facility</option>
                            <option value="House/Apartment/Residence">House/Apartment/Residence</option>
                            <option value="Race/Ethnicity Identified">Race/Ethnicity Identified</option>
                            <option value="Judge/Attorney/Court">Judge/Attorney/Court</option>
                            <option value="Police Patrol (including vehicles and convoys)">Police Patrol (including vehicles and
                                convoys)</option>
                            <option value="Place of Worship">Place of Worship</option>
                            <option value="Vehicles/Transportation">Vehicles/Transportation</option>
                            <option value="Bus (excluding tourists)">Bus (excluding tourists)</option>
                            <option value="Politician or Political Party Movement/Meeting/Rally">Politician or Political Party
                                Movement/Meeting/Rally</option>
                            <option value="Hotel/Resort">Hotel/Resort</option>
                            <option value="Airport">Airport</option>
                            <option value="Bank/Commerce">Bank/Commerce</option>
                            <option value="Construction">Construction</option>
                            <option value="Tour Bus/Van">Tour Bus/Van</option>
                            <option value="Train/Train Tracks/Trolley">Train/Train Tracks/Trolley</option>
                            <option value="Religion Identified">Religion Identified</option>
                            <option value="Gas/Oil/Electric">Gas/Oil/Electric</option>
                            <option value="Newspaper Journalist/Staff/Facility">Newspaper Journalist/Staff/Facility</option>
                            <option value="Military Maritime">Military Maritime</option>
                            <option value="Head of State">Head of State</option>
                            <option value="Entertainment/Cultural/Stadium/Casino">Entertainment/Cultural/Stadium/Casino</option>
                            <option value="Labor Union Related">Labor Union Related</option>
                            <option value="Restaurant/Bar/Café">Restaurant/Bar/Café</option>
                            <option value="Museum/Cultural Center/Cultural House">Museum/Cultural Center/Cultural House</option>
                            <option value="Government Personnel (excluding police, military)">Government Personnel (excluding police,
                                military)</option>
                            <option value="Public Area (garden, parking lot, garage, beach, public building, camp)">Public Area
                                (garden, parking lot, garage, beach, public building, camp)</option>
                            <option value="Industrial/Textiles/Factory">Industrial/Textiles/Factory</option>
                            <option value="International NGO">International NGO</option>
                            <option value="Unnamed Civilian/Unspecified">Unnamed Civilian/Unspecified</option>
                            <option value="Political Party Member/Rally">Political Party Member/Rally</option>
                            <option value="Telephone/Telegraph">Telephone/Telegraph</option>
                            <option value="Airline Officer/Personnel">Airline Officer/Personnel</option>
                            <option value="Water Supply">Water Supply</option>
                            <option value="Laborer (General)/Occupation Identified">Laborer (General)/Occupation Identified</option>
                            <option value="Military Transportation/Vehicle (excluding convoys)">Military Transportation/Vehicle
                                (excluding convoys)</option>
                            <option value="Business">Business</option>
                            <option value="Domestic NGO">Domestic NGO</option>
                            <option value="Radio">Radio</option>
                            <option value="Farm/Ranch">Farm/Ranch</option>
                            <option value="Terrorist">Terrorist</option>
                            <option value="Military Unit/Patrol/Convoy">Military Unit/Patrol/Convoy</option>
                            <option value="Fire Fighter/Truck">Fire Fighter/Truck</option>
                            <option value="Other Personnel">Other Personnel</option>
                            <option value="Procession/Gathering (funeral, wedding, birthday, religious)">Procession/Gathering
                                (funeral, wedding, birthday, religious)</option>
                            <option value="Mining">Mining</option>
                            <option value="Oil">Oil</option>
                            <option value="Medical/Pharmaceutical">Medical/Pharmaceutical</option>
                            <option value="Village/City/Town/Suburb">Village/City/Town/Suburb</option>
                            <option value="Memorial/Cemetery/Monument">Memorial/Cemetery/Monument</option>
                            <option value="International Organization (peacekeeper, aid agency, compound)">International Organization
                                (peacekeeper, aid agency, compound)</option>
                            <option value="Affiliated Institution">Affiliated Institution</option>
                            <option value="Tourism Travel Agency">Tourism Travel Agency</option>
                            <option value="Television">Television</option>
                            <option value="Television Journalist/Staff/Facility">Television Journalist/Staff/Facility</option>
                            <option value="Party Official/Candidate/Other Personnel">Party Official/Candidate/Other Personnel</option>
                            <option value="Highway/Road/Toll/Traffic Signal">Highway/Road/Toll/Traffic Signal</option>
                            <option value="Oil Tanker">Oil Tanker</option>
                            <option value="Other Facility">Other Facility</option>
                            <option value="Tourist">Tourist</option>
                            <option value="Royalty">Royalty</option>
                            <option value="Civilian Maritime">Civilian Maritime</option>
                            <option value="Gas">Gas</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="natlty1_txt">Nationalitate tinta:</label>
                    </div>
                    <div class="filteritem">
                        <select id="natlty1_txt" name="natlty1_txt">
                            <option></option>
                            <option value="Dominican Republic">Dominican Republic</option>
                            <option value="Belgium">Belgium</option>
                            <option value="United States">United States</option>
                            <option value="Uruguay">Uruguay</option>
                            <option value="Germany">Germany</option>
                            <option value="Great Britain">Great Britain</option>
                            <option value="Venezuela">Venezuela</option>
                            <option value="Puerto Rico">Puerto Rico</option>
                            <option value="Israel">Israel</option>
                            <option value="Austria">Austria</option>
                            <option value="Switzerland">Switzerland</option>
                            <option value="Jordan">Jordan</option>
                            <option value="Ethiopia">Ethiopia</option>
                            <option value="Spain">Spain</option>
                            <option value="Japan">Japan</option>
                            <option value="International">International</option>
                            <option value="Egypt">Egypt</option>
                            <option value="Paraguay">Paraguay</option>
                            <option value="Soviet Union">Soviet Union</option>
                            <option value="Ireland">Ireland</option>
                            <option value="Argentina">Argentina</option>
                            <option value="Philippines">Philippines</option>
                            <option value="China">China</option>
                            <option value="West Bank and Gaza Strip">West Bank and Gaza Strip</option>
                            <option value="Multinational">Multinational</option>
                            <option value="Northern Ireland">Northern Ireland</option>
                            <option value="Brazil">Brazil</option>
                            <option value="Haiti">Haiti</option>
                            <option value="South Africa">South Africa</option>
                            <option value="Portugal">Portugal</option>
                            <option value="Colombia">Colombia</option>
                            <option value="Greece">Greece</option>
                            <option value="Nicaragua">Nicaragua</option>
                            <option value="Indonesia">Indonesia</option>
                            <option value="Yugoslavia">Yugoslavia</option>
                            <option value="Italy">Italy</option>
                            <option value="Rhodesia">Rhodesia</option>
                            <option value="Ivory Coast">Ivory Coast</option>
                            <option value="Iran">Iran</option>
                            <option value="Canada">Canada</option>
                            <option value="Poland">Poland</option>
                            <option value="Sweden">Sweden</option>
                            <option value="Turkey">Turkey</option>
                            <option value="Costa Rica">Costa Rica</option>
                            <option value="Russia">Russia</option>
                            <option value="Iraq">Iraq</option>
                            <option value="Cuba">Cuba</option>
                            <option value="France">France</option>
                            <option value="Malawi">Malawi</option>
                            <option value="Democratic Republic of the Congo">Democratic Republic of the Congo</option>
                            <option value="Netherlands">Netherlands</option>
                            <option value="Chile">Chile</option>
                            <option value="Hong Kong">Hong Kong</option>
                            <option value="Yemen">Yemen</option>
                            <option value="Mexico">Mexico</option>
                            <option value="Syria">Syria</option>
                            <option value="India">India</option>
                            <option value="New Zealand">New Zealand</option>
                            <option value="Senegal">Senegal</option>
                            <option value="Algeria">Algeria</option>
                            <option value="Peru">Peru</option>
                            <option value="Czechoslovakia">Czechoslovakia</option>
                            <option value="Saudi Arabia">Saudi Arabia</option>
                            <option value="Denmark">Denmark</option>
                            <option value="West Germany (FRG)">West Germany (FRG)</option>
                            <option value="Hostage Taking (Kidnapping)">Hostage Taking (Kidnapping)</option>
                            <option value="Bahamas">Bahamas</option>
                            <option value="Bolivia">Bolivia</option>
                            <option value="Race/Ethnicity Identified">Race/Ethnicity Identified</option>
                            <option value="El Salvador">El Salvador</option>
                            <option value="Botswana">Botswana</option>
                            <option value="Guatemala">Guatemala</option>
                            <option value="Lebanon">Lebanon</option>
                            <option value="North Korea">North Korea</option>
                            <option value="Albania">Albania</option>
                            <option value="South Korea">South Korea</option>
                            <option value="Greyhound Bus Depot">Greyhound Bus Depot</option>
                            <option value="United Kingdom">United Kingdom</option>
                            <option value="Pakistan">Pakistan</option>
                            <option value="Kenya">Kenya</option>
                            <option value="Cyprus">Cyprus</option>
                            <option value="Panama">Panama</option>
                            <option value="Sri Lanka">Sri Lanka</option>
                            <option value="Ecuador">Ecuador</option>
                            <option value="Malaysia">Malaysia</option>
                            <option value="Morocco">Morocco</option>
                            <option value="Somalia">Somalia</option>
                            <option value="Nigeria">Nigeria</option>
                            <option value="Namibia">Namibia</option>
                            <option value="Armenia">Armenia</option>
                            <option value="Barbados">Barbados</option>
                            <option value="Kuwait">Kuwait</option>
                            <option value="Mozambique">Mozambique</option>
                            <option value="Guyana">Guyana</option>
                            <option value="Taiwan">Taiwan</option>
                            <option value="Tunisia">Tunisia</option>
                            <option value="Myanmar">Myanmar</option>
                            <option value="Trinidad and Tobago">Trinidad and Tobago</option>
                            <option value="Ohio Police">Ohio Police</option>
                            <option value="Mauritania">Mauritania</option>
                            <option value="Bulgaria">Bulgaria</option>
                            <option value="Thailand">Thailand</option>
                            <option value="Bahrain">Bahrain</option>
                            <option value="Finland">Finland</option>
                            <option value="Vietnam">Vietnam</option>
                            <option value="United Arab Emirates">United Arab Emirates</option>
                            <option value="Malta">Malta</option>
                            <option value="Australia">Australia</option>
                            <option value="Zimbabwe">Zimbabwe</option>
                            <option value="Zambia">Zambia</option>
                            <option value="Angola">Angola</option>
                            <option value="Norway">Norway</option>
                            <option value="Jamaica">Jamaica</option>
                            <option value="Uganda">Uganda</option>
                            <option value="Croatia">Croatia</option>
                            <option value="Lesotho">Lesotho</option>
                            <option value="Tanzania">Tanzania</option>
                            <option value="Guinea">Guinea</option>
                            <option value="Fiji">Fiji</option>
                            <option value="Djibouti">Djibouti</option>
                            <option value="Libya">Libya</option>
                            <option value="Eritrea">Eritrea</option>
                            <option value="Afghanistan">Afghanistan</option>
                            <option value="Honduras">Honduras</option>
                            <option value="Zaire">Zaire</option>
                            <option value="Mali">Mali</option>
                            <option value="Bangladesh">Bangladesh</option>
                            <option value="Grenada">Grenada</option>
                            <option value="Vanuatu">Vanuatu</option>
                            <option value="Belize">Belize</option>
                            <option value="North Yemen">North Yemen</option>
                            <option value="Romania">Romania</option>
                            <option value="Sudan">Sudan</option>
                            <option value="Chad">Chad</option>
                            <option value="Central African Republic">Central African Republic</option>
                            <option value="Liberia">Liberia</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="weaptype1_txt">Arma principala:</label>
                    </div>
                    <div class="filteritem">
                        <select id="weaptype1_txt" name="weaptype1_txt">
                            <option></option>
                            <option value="Unknown">Unknown</option>
                            <option value="Explosives">Explosives</option>
                            <option value="Incendiary">Incendiary</option>
                            <option value="Firearms">Firearms</option>
                            <option value="Chemical">Chemical</option>
                            <option value="Melee">Melee</option>
                            <option value="Sabotage Equipment">Sabotage Equipment</option>
                            <option value="Vehicle">Vehicle</option>
                            <option value="Fake Weapons">Fake Weapons</option>
                            <option value="Radiological">Radiological</option>
                            <option value="Other">Other</option>
                            <option value="Biological">Biological</option>
                            <option value="Letter">Letter</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="weapsubtype1_txt">Arma secundara:</label>
                    </div>
                    <div class="filteritem">
                        <select id="weapsubtype1_txt" name="weapsubtype1_txt">
                            <option></option>
                            <option value="Unknown Explosive Type">Unknown Explosive Type</option>
                            <option value="Unknown Gun Type">Unknown Gun Type</option>
                            <option value="Automatic or Semi-Automatic Rifle">Automatic or Semi-Automatic Rifle</option>
                            <option value="Molotov Cocktail/Petrol Bomb">Molotov Cocktail/Petrol Bomb</option>
                            <option value="Gasoline or Alcohol">Gasoline or Alcohol</option>
                            <option value="Rifle/Shotgun (non-automatic)">Rifle/Shotgun (non-automatic)</option>
                            <option value="Arson/Fire">Arson/Fire</option>
                            <option value="Pipe Bomb">Pipe Bomb</option>
                            <option value="Vehicle">Vehicle</option>
                            <option value="Dynamite/TNT">Dynamite/TNT</option>
                            <option value="Pressure Trigger">Pressure Trigger</option>
                            <option value="Time Fuse">Time Fuse</option>
                            <option value="Other Explosive Type">Other Explosive Type</option>
                            <option value="Poisoning">Poisoning</option>
                            <option value="Projectile (rockets, mortars, RPGs, etc.)">Projectile (rockets, mortars, RPGs, etc.)
                            </option>
                            <option value="Handgun">Handgun</option>
                            <option value="Letter Bomb">Letter Bomb</option>
                            <option value="Explosives">Explosives</option>
                            <option value="Blunt Object">Blunt Object</option>
                            <option value="Sticky Bomb">Sticky Bomb</option>
                            <option value="Grenade">Grenade</option>
                            <option value="Knife or Other Sharp Object">Knife or Other Sharp Object</option>
                            <option value="Landmine">Landmine</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="nkillin">Numar minim victime:</label>
                    </div>
                    <div class="filteritem">
                        <input type="number" id="nkillin" name="nkillin">
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="nkillsf">Numar maxim victime:</label>
                    </div>
                    <div class="filteritem">
                        <input type="number" id="nkillsf" name="nkillsf">
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="propextent_txt">Daune provocate:</label>
                    </div>
                    <div class="filteritem">
                        <select id="propextent_txt" name="propextent_txt">
                            <option></option>
                            <option value="Minor (likely < $1 million)">Minor (likely < $1 million)</option>
                            <option value="Major (likely >= $1 million but < $1 billion)">Major (likely >= $1 million but < $1 billion)</option>
                            <option value="Unknown">Unknown</option>
                            <option value="Catastrophic (likely >= $1 billion)">Catastrophic (likely >= $1 billion)</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="ransom">Rascumparare (?):</label>
                    </div>
                    <div class="filteritem">
                        <input type="number" id="ransom" name="ransom" min="0" max="1">
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="ransomamtin">Taxa minima de rascumparare:</label>
                    </div>
                    <div class="filteritem">
                        <input type="number" id="ransomamtin" name="ransomamtin">
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="ransomamtsf">Taxa maxima de rascumparare:</label>
                    </div>
                    <div class="filteritem">
                        <input type="number" id="ransomamtsf" name="ransomamtsf">
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="selecteazagr">Selecteaza grafic</label>
                    </div>
                    <div class="filteritem">
                        <select id="selecteazagr" name="selecteazagr">
                            <option value="Pie">Pie</option>
                            <option value="Bar">Bar</option>
                            <option value="Line">Line</option>
                        </select>
                    </div>
                </div>

                <div class="filterline">
                    <div class="filterlabel">
                        <label for="tipdategrafic">Creaza grafic dupa:</label>
                    </div>
                    <div class="filteritem">
                        <select id="tipdategrafic" name="tipdategrafic">
                            <option value="iyear">An</option>
                            <option value="country_txt">Tara</option>
                            <option value="region_txt">Regiune</option>
                            <option value="provstate">Judet</option>
                            <option value="attacktype1_txt">Tip atac</option>
                            <option value="targtype1_txt">Tinta principala</option>
                            <option value="targsubtype1_txt">Tinta secundara</option>
                            <option value="natlty1_txt">Nationalitate tinta</option>
                            <option value="weaptype1_txt">Arma principala</option>
                            <option value="weapsubtype1_txt">Arma secundara</option>
                            <option value="propextent_txt">Daune provocate</option>
                            <option value="ransom">Rascumparare ceruta?</option>
                        </select>
                    </div>
                </div>

            </div>
        </form>

        <aside>

            <article class="articol">
                <h4>
                    Date: 12/31/2017
                </h4>
                <h4>
                    An explosive device was discovered and defused at a plaza in Cotabato City, Maguindanao,
                    Philippines. No group claimed responsibility for the incident.
                </h4>
                <h4>
                    <a href="#" class="butonSee">See More</a>
                </h4>

            </article>

            <article class="articol">
                <h4>
                    Date: 12/31/2017
                </h4>
                <h4>
                    Assailants threw a grenade at a Forest Department office in Mantripukhri neighborhood, Imphal,
                    Manipur, India. No casualties were reported in the blast. No group claimed responsibility for the
                    incident.
                </h4>
                <h4>
                    <a href="#" class="butonSee">See More</a>
                </h4>
            </article>

            <article class="articol">
                <h4>
                    Date: 12/31/2017
                </h4>
                <h4>
                    Assailants set fire to houses in Kubentog, Datu Hoffer, Maguindanao, Philippines. There were no
                    reported casualties in the attack. No group claimed responsibility for the incident; however,
                    sources attributed the attack to the Bangsamoro Islamic Freedom Movement (BIFM).
                </h4>
                <h4>
                    <a href="#" class="butonSee">See More</a>
                </h4>
            </article>

            <article class="articol">
                <h4>
                    Date: 12/31/2017
                </h4>
                <h4>
                    Assailants launched mortars at the Hmeymim Air Base in Jableh, Lattakia, Syria. Two Russian soldiers
                    were killed and ten people were injured in the attack. No group claimed responsibility for the
                    incident; however, sources attributed the attack to Muslim extremists.
                </h4>
                <h4>
                    <a href="#" class="butonSee">See More</a>
                </h4>
            </article>

            <article class="articol">
                <h4>
                    Date: 01/01/1998
                </h4>
                <h4>
                    Hutu Rebels attacked a Burundi military target at Bujumbura Airport. The attack resulted in 104
                    fatalities, including 100 perpetrators, and six injuries.
                </h4>
                <h4>
                    <a href="#" class="butonSee">See More</a>
                </h4>
            </article>

        </aside>

        <div id="Pie">

        </div>

        <div id="PieLeg">

        </div>

        <div id="Bar">

        </div>

        <div id="Line">

        </div>


    </main>
</body>

</html>