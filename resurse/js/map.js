  ...
  fetch(url)
    ...
    .then(function (jsonResp) {
      if (jsonResp.hasOwnProperty('response')) {
        if (jsonResp.response.includes("Invalid")) { var divGol = document.createElement("div"); divGol.setAttribute("class", "textContainer"); divGol.textContent = jsonResp.response; container.appendChild(divGol);}
      }
      else {
        drawMap(jsonResp);
      }
